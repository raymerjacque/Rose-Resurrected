...
bool CCharServer::lma_mask (CCharClient* thisclient)
{
    ...
    if( thisclient->clanid > 0 )
    {
        ...
        if(thisclan!=NULL)
        {
            ...
            ADDWORD    ( pak, 0x0000); //??
            ADDQWORD   ( pak, thisclan->money );   //Clan Money
            ADDDWORD    ( pak, 0xcccccccc);  //Nb of clan members.
            ADDWORD    ( pak, 0xcccc);
            for(int i=0;i<60;i++)
                ADDWORD ( pak, thisclan->skills[i]);
        ...
        case 0xfa://message from worldserver to load the new clan information
        {
           ...
           if(otherclient==NULL)
        	...
        	// Load all our clan information
        	            //     0    1  2    3    4   5   6       7
        	result = DB->QStore("SELECT id,logo,back,name,cp,grade,slogan,news,rankingpoints,siglogo,money,skills FROM list_clan where id=%i",id);
            if(result == NULL) return false;
            if(mysql_num_rows(result)!=1)
            {
                DB->QFree( );
            ...
            newclan->money = (row[10] != NULL) ? strtoull(row[10], NULL, 10) : 0;
            memset(newclan->skills, 0, sizeof(newclan->skills));
            if (row[11] != NULL && strlen(row[11]) > 0)
            {
                char skills_buf[512];
                strncpy(skills_buf, row[11], sizeof(skills_buf) - 1);
                skills_buf[sizeof(skills_buf) - 1] = '\0';
                char* token = strtok(skills_buf, ",");
                int s_idx = 0;
                while (token != NULL && s_idx < 60)
                {
                    newclan->skills[s_idx++] = (short)atoi(token);
                    token = strtok(NULL, ",");
                }
            }
            //DB->QFree( );

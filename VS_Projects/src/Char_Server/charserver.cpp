...
bool CCharServer::OnServerReady( )
{
	...
	// Load all our clans
	result = DB->QStore("SELECT id,logo,back,name,cp,grade,slogan,news,rankingpoints,siglogo,money,skills FROM list_clan");
	if(result==NULL)  return false;
	while(row = mysql_fetch_row(result))
    {
        ...
        thisclan->money = (row[10] != NULL) ? strtoull(row[10], NULL, 10) : 0;
        memset(thisclan->skills, 0, sizeof(thisclan->skills));
        if (row[11] != NULL && strlen(row[11]) > 0)
        {
            ...
            {
                thisclan->skills[s_idx++] = (short)atoi(token);
                token = strtok(NULL, ",");
            }
        }

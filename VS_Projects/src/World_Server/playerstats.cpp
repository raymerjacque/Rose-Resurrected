...
void CPlayer::ApplyClanPassive(UINT buffType1, UINT buffType2, UINT buffType3, UINT &pVal, UINT &vVal)
{
    if(Clan == NULL || Clan->clanid == 0) return;
    for(int s=0; s<60; s++)
    {
        if(Clan->skills[s] == 0) continue;
        CSkills* cskill = GServer->GetSkillByID(Clan->skills[s]);
        if(cskill == NULL || cskill->type != 15) continue;
        for(UINT j=0; j<3; j++)
        {
            if((buffType1 != 0 && cskill->buff[j] == buffType1) ||
               (buffType2 != 0 && cskill->buff[j] == buffType2) ||
               (buffType3 != 0 && cskill->buff[j] == buffType3))
            {
                if(cskill->value2[j] > 0)
                    pVal += cskill->value2[j];
                if(cskill->value1[j] > 0)
                    vVal += cskill->value1[j];
            }
        }
    }
}
...
unsigned int CPlayer::GetDodge( )
{
    ...
    if(Status->Stance != DRIVING)       //Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(items[i].gem!=0)
                {
                    if(GServer->JemList.Index.at(items[i].gem)!=NULL)
                    {
                        if(GServer->JemList.Index.at(items[i].gem)->stat1[0] == A_DODGE || GServer->JemList.Index.at (items[i].gem)->stat1[0] == DODGE)
                        {
                            Dodge += GServer->JemList.Index.at(items[i].gem)->stat1[1];
                            Dodge += (UINT)floor(Dodge * GServer->JemList.Index.at (items[i].gem)->stat1[2] * 0.01);
                        }
                        if(GServer->JemList.Index.at(items[i].gem)->stat2[0] == A_DODGE || GServer->JemList.Index.at (items[i].gem)->stat2[0] == DODGE)
                        {
                            Dodge += GServer->JemList.Index.at(items[i].gem)->stat2[1];
                            Dodge += (UINT)floor(Dodge * GServer->JemList.Index.at(items[i].gem)->stat2[2] * 0.01);
                        }
                    }
                }
                //End gem

                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_DODGE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == DODGE)
                {
                    Dodge += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_DODGE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == DODGE)
                {
                    Dodge += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    ApplyClanPassive(DODGE, A_DODGE, 0, pDodge, vDodge);
...
unsigned int CPlayer::GetAccuracy( )
{
    ...
    if(Status->Stance != DRIVING)       //Walking, Running, Fighting, ...  Stats
    {
        ...
        else if(items[7].count !=0)
        {
            ...
            int itemAcc = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->acc;
        ...
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_ACCUR || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == ATK_ACCURACY)
                {
                    Accuracy += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_ACCUR || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == ATK_ACCURACY)
                {
                    Accuracy += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
            ...
            if(items[i].count>0)
            {
                ...
                if(GServer->PatList.Index[items[i].itemnum]->attackpower > 0)//If Weapon PAT Attackpower > 0 -> PAT weapon, if PAT weapon -> acc
                {
                    ...
                    switch (GServer->PatList.Index[items[i].itemnum]->type)
                    {
                        ...
                        break;
                        case 551://Battle Machine Gun, Battle Launcher (legacy)
                        {
                            switch(GServer->PatList.Index[items[i].itemnum]->id)
                            {
                                case 311:
    ...
    ApplyClanPassive(ATK_ACCURACY, A_ACCUR, 0, pAccuracy, vAccuracy);
...
unsigned int CPlayer::GetCritical( )
{
    ...
    if(Status->Stance != DRIVING)       //Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_CRITICAL || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == CRITICAL)
                {
                    Critical += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_CRITICAL || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == CRITICAL)
                {
                    Critical += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    ApplyClanPassive(A_CRITICAL, CRITICAL, 0, pCritical, vCritical);
...
{
    if(GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->magicattack == 1)
        return 1;
    else
        return 0;
}
...
unsigned int CPlayer::GetMagicDefense( )
{
    ...
    for(UINT i=1;i<9;i++)//Refine
    {
        ...
        if(items[i].count>0)
        {
            ...
            MagicDefense += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->magicresistence;
            ...
            if(items[i].refine>0)//MDeff From Refine
            {
                ...
                if(refine>0 && refine<GServer->maxGrades)
                {
                    ...
                    MagicDefense += (UINT)floor(GServer->GradeList[refine]->mdef_percent * 0.01 * GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->magicresistence );
                }
            }
        }
    }
    ...
    if(Status->Stance != DRIVING)       //Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_MRESIST || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MAGIC_RESISTENCE_2)
                {
                    MagicDefense += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_MRESIST || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MAGIC_RESISTENCE_2)
                {
                    MagicDefense += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    ApplyClanPassive(A_MRESIST, MAGIC_RESISTENCE_2, 0, pMagicDefense, vMagicDefense);
...
unsigned int CPlayer::GetAttackPower( )
{
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        if(items[7].itemnum != 0 && items[7].count > 0 && items[7].durability > 0)
        {
            ...
            weaponatk = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->attackpower;
            weapontype = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->type;
        ...
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_ATTACK)
                {
                    attack += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_ATTACK)
                {
                    attack += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
        ...
        ApplyClanPassive(A_ATTACK, 0, 0, pattack, vattack);
        ...
        for(UINT i=135;i<140;i++)//PAT Stats
        {
            if(items[i].count>0)
            {
                ...
                if(  GServer->PatList.Index[items[i].itemnum]->attackpower != 0 && items[7].count == 0 ||
                    (GServer->PatList.Index[items[i].itemnum]->attackpower != 0 && items[7].count > 0 && GServer->EquipList[items[7].itemtype].itemList.at (items[7].itemnum)->id == 1))
                {
                    ...
                    if( GServer->PatList.Index[items[i].itemnum]->type == 571 || GServer->PatList.Index[items[i].itemnum]->type == 572 || GServer->PatList.Index[items[i].itemnum]->type == 573 ||
                        GServer->PatList.Index[items[i].itemnum]->type == 574 || GServer->PatList.Index[items[i].itemnum]->type == 575 || GServer->PatList.Index[items[i].itemnum]->type == 551 )
                    {
                ...
                else if(GServer->PatList.Index[items[i].itemnum]->attackpower != 0 && items[7].count > 0 && GServer->EquipList[items[7].itemtype].itemList.at (items[7].itemnum)->id > 1)
                {
                    UINT bonusjob = 0;
                    double bonuslvl = 0;
                    double bonusstr = 0;
                    ...
                    switch (GServer->PatList.Index[items[i].itemnum]->type)//We check the type of the PAT Weapon because some Weapon can be painted and we will not need to add each id for each color
                    {
                        ...
                        case 571://Stone Hammer, Doom Hammer
                        {
                            attack += (UINT)floor( (Stats->Level * 0.35) + (Attr->Str * 0.75) + (Attr->Con * 0.40) + (weaponatk * 0.15) );
                            attack += GServer->PatList.Index[items[i].itemnum]->attackpower;
                        }
                        break;
                        case 572://Battle Mirror, Battle Crystal
                        {
                            attack += (UINT)floor( (Stats->Level * 0.35) + (Attr->Int * 0.85) + (Attr->Sen * 0.45) + (weaponatk * 0.15) );
                            attack += GServer->PatList.Index[items[i].itemnum]->attackpower;
                        }
                        break;
                        case 573://Battle Bow, Tarkion Bow
                        {
                            attack += (UINT)floor( (Stats->Level * 0.35) + (Attr->Dex * 0.85) + (Attr->Sen * 0.40) + (weaponatk * 0.15) );
                            attack += GServer->PatList.Index[items[i].itemnum]->attackpower;
                        }
                        break;
                        case 574://Battle Katar, Battle Spear
                        {
                            attack += (UINT)floor( (Stats->Level * 0.35) + (Attr->Str * 0.55) + (Attr->Dex * 0.65) + (weaponatk * 0.15) );
                            attack += GServer->PatList.Index[items[i].itemnum]->attackpower;
                        }
                        break;
                        case 575:
                        case 551://Battle Machine Gun, Battle Launcher
                        {
                            attack += (UINT)floor( (Stats->Level * 0.35) + (Attr->Con * 0.65) + (Attr->Str * 0.45) + (Attr->Dex * 0.35) + (weaponatk * 0.15) );
                            attack += GServer->PatList.Index[items[i].itemnum]->attackpower;
                        }
                        break;
                        default://Other PAT Weapon
                        {
                            attack += (UINT)floor( (Stats->Level * 0.35) + (Attr->Str * 0.50) + (Attr->Dex * 0.50) + (weaponatk * 0.15) );
                            attack += GServer->PatList.Index[items[i].itemnum]->attackpower;
                        }
                        break;
...
unsigned int CPlayer::GetDefense( )
{
    ...
    for(UINT i=1;i<9;i++)//Refine
    {
        ...
        if(items[i].count>0)
        {
            ...
            {
                subweapdef += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->defense;
            }

            itemdef += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->defense;
            ...
            if(items[i].refine>0)//Deff From Refine
            {
                ...
                if(refine>0 && refine<GServer->maxGrades)
                {
                    ...
                    itemdef += (UINT)floor(GServer->GradeList[refine]->def_percent * 0.01 * GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->defense );
                    if(items[i].itemtype == SUBWEAPON)
                    {
                        subweapdef += GServer->GradeList[refine]->def_addbonus;
                        subweapdef += (UINT)floor(GServer->GradeList[refine]->def_percent * 0.01 * GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->defense );
                    }
                }
            }
        }
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_DEFENSE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == DEFENSE)
                {
                    itemdef += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_DEFENSE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == DEFENSE)
                {
                    itemdef += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    ApplyClanPassive(DEFENSE, A_DEFENSE, 0, pdefense, vdefense);
...
UINT CPlayer::GetAttackSpeed( )
{
    ...
    {
        UINT wpnspd = 0;
        wpnspd = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->attackspeed;
...
float CPlayer::GetAttackSpeedPercent()
{
    ...
    UINT weapontype = 0;
    weapontype = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->type;
	paspeed = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->attackspeed;
    ...
    UINT dummy_aspeed_v = 0;
    UINT clan_haste = 0;
    ApplyClanPassive(A_HASTE, 0, 0, clan_haste, dummy_aspeed_v);
    paspeed += clan_haste;
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
	{
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_HASTE)
                {
                    paspeed += ((GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1])*0.3);
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_HASTE)
                {
                    paspeed += ((GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1])*0.3);
                }
...
unsigned int CPlayer::GetMoveSpeed( )
{
    ...
    switch(Status->Stance)
    {
        ...
        case RUNNING: // running
        {
            ...
            {
                UINT tspeed = GServer->EquipList[SHOE].itemList.at (items[6].itemnum)->movespeed;
        		mspeed += ( tspeed - 65 ) * 5;
            }
            ...
            for(UINT i=0;i<12;i++)//Cloth Stats
            {
                if(items[i].count>0)
                {
                    ...
                    if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_DASH || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MOV_SPEED)
                    {
                        mspeed += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                    }
                    if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_DASH || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MOV_SPEED)
                    {
                       mspeed += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                    }
            ...
            ApplyClanPassive(MOV_SPEED, A_DASH, 0, pmspeed, vmspeed);
...
unsigned long long CPlayer::GetMaxHP( )
{
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_MAX_HP || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_HP ||
                    GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MAX_HP)
                {
                    hpmax += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_MAX_HP || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_HP ||
                    GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MAX_HP)
                {
                    hpmax += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    ApplyClanPassive(A_MAX_HP, A_HP, MAX_HP, phpmax, vhpmax);
...
unsigned int CPlayer::GetMaxMP( )
{
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_MAX_MP || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_MP ||
                    GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MAX_MP)
                {
                    maxmp += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_MAX_MP || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_MP ||
                    GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MAX_MP)
                {
                    maxmp += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    ApplyClanPassive(A_MAX_MP, A_MP, MAX_MP, pmaxmp, vmaxmp);
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)              //Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_HP_REC_RATE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == HP_REC_AMONT)
                {
                    amount += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_HP_REC_RATE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == HP_REC_AMONT)
                {
                    amount += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_MP_REC_RATE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MP_REC_RATE)
                {
                    amount += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_MP_REC_RATE || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MP_REC_RATE)
                {
                    amount += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == A_INVENTORY_CAPACITY || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == BAGPACK_CAPACITY)
                {
                    weight += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == A_INVENTORY_CAPACITY || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == BAGPACK_CAPACITY)
                {
                    weight += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
...
unsigned int CPlayer::GetCurrentWeight( )
{
    ...
    for(UINT i=0;i<MAX_INVENTORY;i++)
    {
        ...
        {
            weight += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->weight;
        }
        else
        {
    ...
    if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MP_COST_RED || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == MP_CONSUME)
                {
                    mpreduction += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MP_COST_RED || GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == MP_CONSUME)
                {
                    mpreduction += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
    ...
    if(Status->Stance != DRIVING)
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == SUMMON_GAUGE)
                {
                    gauge += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
                }
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == SUMMON_GAUGE)
                {
                    gauge += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
                }
...
void CPlayer::GetExtraStats( )
{
    ...
    if (Status->Stance != DRIVING)//When Driving Extra Attributes is not added
    {
        for(UINT i=1;i<12;i++)//Cloth Stats
        {
            if(items[i].count>0)
            {
                UINT Stat1 = GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0];
                UINT Stat2 = GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0];

                switch(Stat1)
                {
                    case A_STR: Attr->Estr += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1]; break;
                    case A_DEX: Attr->Edex += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1]; break;
                    case A_INT: Attr->Eint += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1]; break;
                    case A_CON: Attr->Econ += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1]; break;
                    case A_CHA: Attr->Echa += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1]; break;
                    case A_SEN: Attr->Esen += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1]; break;
                }
                switch(Stat2)
                {
                    case A_STR: Attr->Estr += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1]; break;
                    case A_DEX: Attr->Edex += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1]; break;
                    case A_INT: Attr->Eint += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1]; break;
                    case A_CON: Attr->Econ += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1]; break;
                    case A_CHA: Attr->Echa += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1]; break;
                    case A_SEN: Attr->Esen += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1]; break;
                }
...
void CPlayer::GetExtraStatsSkills( )
{
    ...
    if (Clan != NULL && Clan->clanid != 0)
    {
        for (int s = 0; s < 60; s++)
        {
            ...
            for (UINT j = 0; j < 3; j++)
            {
                switch (cskill->buff[j])
                {
                    case A_STR:
                        if (cskill->value2[j] > 0) Extra[0] += Attr->Estr * cskill->value2[j] / 100;
                        if (cskill->value1[j] > 0) Extra[0] += cskill->value1[j];
                        break;
                    case A_DEX:
                        if (cskill->value2[j] > 0) Extra[1] += Attr->Edex * cskill->value2[j] / 100;
                        if (cskill->value1[j] > 0) Extra[1] += cskill->value1[j];
                        break;
                    case A_INT:
                        if (cskill->value2[j] > 0) Extra[2] += Attr->Eint * cskill->value2[j] / 100;
                        if (cskill->value1[j] > 0) Extra[2] += cskill->value1[j];
                        break;
                    case A_CON:
                        if (cskill->value2[j] > 0) Extra[3] += Attr->Econ * cskill->value2[j] / 100;
                        if (cskill->value1[j] > 0) Extra[3] += cskill->value1[j];
                        break;
                    case A_CHA:
                        if (cskill->value2[j] > 0) Extra[4] += Attr->Echa * cskill->value2[j] / 100;
                        if (cskill->value1[j] > 0) Extra[4] += cskill->value1[j];
                        break;
                    case A_SEN:
                        if (cskill->value2[j] > 0) Extra[5] += Attr->Esen * cskill->value2[j] / 100;
                        if (cskill->value1[j] > 0) Extra[5] += cskill->value1[j];
                        break;
                }
            }
        }
...
float CPlayer::GetAttackDistance( )
{
    ...
    if(Status->Stance == DRIVING)//PAT Stats
	{
        for(UINT i=135;i<140;i++)//PAT Stats
        {
            if(items[i].count>0)
            {
                ...
                {
                    AttackDistance += (float)GServer->PatList.Index[items[i].itemnum]->attackdistance / 100.0f;
    ...
    else if(Status->Stance != DRIVING)//Walking, Running, Fighting, ...  Stats
    {
        ...
        AttackDistance = (float)GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->attackdistance / 100.0f;
    }
...
unsigned int CPlayer::GetXPRate( )
{
    ...
    for(UINT i=1;i<12;i++)
    {
        if( items[i].count != 0 )
        {
            ...
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == 30) //30 = XP Rate
                XPRate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == 30)
                XPRate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
...
unsigned int CPlayer::GetItemDropRate( )
{
    ...
    for(UINT i=1;i<12;i++)
    {
        if( items[i].count != 0 )
        {
            ...
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == 134 ) //ITEM_DROP_RATE
                itemdroprate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == 134 ) //ITEM_DROP_RATE
                itemdroprate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
            if(items[i].stats > 0 && items[i].stats < 500)
            {
...
unsigned int CPlayer::GetZulyDropRate( )
{
    ...
    for(UINT i=1;i<12;i++)
    {
        if( items[i].count != 0 )
        {
            ...
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == 64 ) //ZULY_DROP_RATE
                zulydroprate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == 64 ) //ZULY_DROP_RATE
                zulydroprate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
            if(items[i].stats > 0 && items[i].stats < 500)
            {
    ...
    if(Status->Stance != DRIVING)
    {
        for(UINT i=1;i<12;i++) // Will leave this loop to look for block rate substats on all items if in future it's placed in the game client
        {
            if(items[i].count>0)
            {
                ...
                if(GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->BlockRate){
                    itemBlock_Rate += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->BlockRate/100;
                } else {
...
unsigned int CPlayer::GetDropAmount( )
{
    ...
    for(UINT i=1;i<12;i++)
    {
        if( items[i].count != 0 )
        {
            ...
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[0] == 63 )
                DropAmount += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat1[1];
            if( GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[0] == 63 )
                DropAmount += GServer->EquipList[items[i].itemtype].itemList.at (items[i].itemnum)->stat2[1];
            if(items[i].stats > 0 && items[i].stats < GServer->maxStats && items[i].gem == 0)
            {
...
void CPlayer::SetStats( )
{
	...
	//Stats->Attack_Power = 10;						//temporary test code to limit attack power
...
void CPlayer::GetDiscount()
{
    ...
    for (int i = 0; i < MAX_ALL_SKILL; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            {
                discount += cskills[i].thisskill->value2[j];
            }
        }
    }
}

void CPlayer::GetMechandising()
{
    merchandising = 0;
    for (int i = 0; i < MAX_ALL_SKILL; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            if (cskills[i].thisskill->buff[j] == 60)
            {
                merchandising += cskills[i].thisskill->value2[j];
            }
        }
    }
}

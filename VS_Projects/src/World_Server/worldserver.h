...
//PY Gate Hacking. Just in case it drives you nuts
//#define DISABLE_GATE_HACKING_PROTECTION
...
class CWorldServer : public CServerSocket
{
    public:
        ...
        //LMA End
      	...
      	//20070621-211100
      	//FOR CF
      	//bool GiveExp( CMonster* thismon, UINT special_lvl, UINT special_exp);  //PY Nope
      	//LMA END

		//PY version of GiveExp that actually works
		bool GiveExp (CMonster* thismon);
		...
		bool isValidSlot			(CPlayer* thisclient, int slot);
		...
		//------------------ Economy stuff
		uint32 GetItemRefinePrice	(CItem* thisItem);							//  FK: Calculate Server-Sided Success Percentage;
		uint16 GetItemRefineProb	(CItem* thisItem);							// Percentage defined by STB
		uint16 GetItemCraftProb		(CPlayer* thisclient, CItem* CraftItem);	// economy.cpp  FK: Calculate Craft Success Percentage.
		uint32 GetItemBuyPrice		(CItem thisitem, int buyskilllevel);		// economy.cpp	FK: Calculate the Price of items bought from NPC;
		uint32 GetItemSellPrice		(CItem thisitem, int buyskilllevel);		// economy.cpp	FK: Calculate the Price of items sold to 
		uint32 GetItemRepairPrice	(CItem* thisitem);							// economy.cpp  FK: Get Item Repair Price quote (from NPC)
		int32 getItemRate			(uint8 itemtype, uint16 itemnum);			// economy.cpp  FK: One universal function to get the item STB price;
		int32 getWorldRate ();													// economy.cpp  FK: One universal entry point;
		int32 getTownRate ();													// economy.cpp  FK: One universal entry point;
...
//        bool LearnSkill( CPlayer* thisclient, UINT skill );
		...
		bool GiveSkill (CPlayer* thisclient, UINT skill, bool takeSP);
		...
		bool doRefineItem (CPlayer* thisclient, BYTE ItemSlot, BYTE Mat1Slot = 0, BYTE Mat2Slot = 0, BYTE Mat3Slot = 0, BYTE Mat4Slot = 0, WORD Mat1Count = 0, WORD Mat2Count = 0, WORD Mat3Coumt = 0, WORD Mat4Count = 0);
		...
		bool pakSetGroup (char* name, WORD groupid);
    	bool pakGMMoveTo( CPlayer* thisclient, fPoint position );
    	bool pakGMTeleAllHere( CPlayer* thisclient );
		bool pakTeleGroupToMe (CPlayer* thisclient, int groupid);
		bool pakTeleGroup (CPlayer* thisclient, WORD map, WORD groupid);
		bool pakStatusGroup (CPlayer* thisclient, int groupid);
		...
		bool pakSetNPCDir (CPlayer* thisclient, CNPC* thisNPC, float dir);
		void pakMoveNPC (CPlayer* thisclient, CNPC* thisNPC);
		void pakSaveNPCDir (CNPC* thisNPC);
		void pakSaveNPCPos (CNPC* thisNPC);
		void pakAddNPC (CPlayer* thisclient, CNPC* thisNPC);
		...
		bool pakBuffGroup (CPlayer* thisClient, WORD groupid, WORD buffPower);
		...
		//----------------- Giving skills
		bool GiveBasicSkills (CPlayer* thisclient);
		bool GiveSoldierSkills (CPlayer* thisclient);
		bool GiveKnightSkills (CPlayer* thisclient);
		bool GiveChampionSkills (CPlayer* thisclient);
		bool GiveMuseSkills (CPlayer* thisclient);
		bool GiveMageSkills (CPlayer* thisclient);
		bool GiveClericSkills (CPlayer* thisclient);
		bool GiveDealerSkills (CPlayer* thisclient);
		bool GiveArtisanSkills (CPlayer* thisclient);
		bool GiveBorgeousSkills (CPlayer* thisclient);
		bool GiveHawkerSkills (CPlayer* thisclient);
		bool GiveScoutSkills (CPlayer* thisclient);
		bool GiveRaiderSkills (CPlayer* thisclient);
		bool GiveGMSkills (CPlayer* thisclient);
		bool GiveDevSkills (CPlayer* thisclient);
		bool GiveDrivingSkills (CPlayer* thisclient);
        ...
        //bool LoadUpgrade( );
		bool LoadRefineChances ();		//PY the proper way to set refine chances
        bool LoadNPCs( );
		...
		bool LoadGoList ();		//PY loads the go list data
		...
		void UpdatePartyExp (CParty* party);
        bool DoSkillScript( CCharacter* character, CSkills* thisskill );
#ifdef USE_137_PARTY_EXP
		UINT AutoExpDecrease = 10000;
		int	GetAutoDecreaseExp (int PartyLevel, int memberCount);
#endif
		...
		//CRecipeList				RecipeList;				
		vector <CCraftRecipe*>	RecipeList;				//PY and Frans. New way of doing recipes
        CSellList               SellList;
        CUseList                UseList;
        CMapList                MapList;
       ...
       // geo edit for disassemble // 22 oct 07
		...
		//Go List stuff. Moved to the database
		struct GoCommand
		{
			UINT Go_id			= 0;
			UINT Mapid			= 0;

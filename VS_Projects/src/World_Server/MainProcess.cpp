...
PVOID MapProcess( PVOID TS )
{
    ...
    while(GServer->ServerOnline)
    {
        ...
        for(UINT i=0;i<GServer->MapList.Map.size();i++)
        {
            ...
            if (!only_npc||only_summon)
            {
                ...
                for(UINT j=0;j<map->PlayerList.size();j++)
                {
                    ...
                    player->CheckDelayedQuestTriggers( );
                ...
                for(UINT j=0;j<map->MonsterList.size();j++)
                {
                   ...
                   //PY utterly pointless. UpdateValues always returns true
                    ...
                    {
                        monster->SummonUpdate(monster,map, j);
                        //continue;		//PY removed calls to deletemonster so this needs to go through the IsDead check
                    }
                    ...
                    if(monster->IsDead( ))
                    {
                        ...
                        if(monster->montype != 1830)
                        {
							...
							if(!monster->IsSummon())
							{ 
								monster->DoAi(monster->thisnpc->AI, 5);	//PY We don't do AI on summons when they die
							}
                        }
                        else
                        {
                        ...
                        monster->OnDie( );				// this just gives XP and quest rewards
						map->DeleteMonster (monster, true, j);
                    }
                }
            }
		...
		// PY Party updates
		for (UINT i = 0; i < GServer->PartyList.size (); i++)
		{
			CParty* party = GServer->PartyList.at (i);
			if (party == NULL)
				continue;
			GServer->UpdatePartyExp (party);
		}
#endif
...
PVOID VisibilityProcess(PVOID TS)
{
    while(GServer->ServerOnline)
    {
        ...
        for(UINT i=0;i<GServer->MapList.Map.size();i++)
        {
            ...
            {
                CPlayer* player = map->PlayerList.at(j);
                if(!player->Session->inGame || player->is_bot)
                {
                    continue;
                }

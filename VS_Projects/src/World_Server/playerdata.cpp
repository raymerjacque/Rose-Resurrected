...
//teams groups
	group					= atoi (row[51]);	//PY: Used for managed sessions. teleport entire groups via GM commands
    ...
    for (int i=0;i<MAX_ALL_SKILL;i++)
    {
        ...
        for(int j=0;j<3;j++)
        {
            ...
            {
                summon_gauge += cskills[i].thisskill->value1[j];
                continue;
            }
    ...
    //Log(MSG_INFO,"%s:: summon gauge %i",CharInfo->charname,summon_gauge);

	result = GServer->DB->QStore("SELECT itemnum, itemtype, refine, durability, lifespan, slotnum, count, stats, socketed, appraised, gem, sp_value, restriction, usetype FROM items WHERE owner = %i", CharInfo->charid);
    if(result == NULL) return false;
	while(row = mysql_fetch_row(result))
    {
		...
		UINT slot = atoi(row[5]);		//PY first we read the slot number of the item
		items[slot].itemnum			= atoi (row[0]);
		items[slot].itemtype		= atoi (row[1]);
		items[slot].refine			= atoi (row[2]);
		items[slot].durability		= atoi (row[3]);
		items[slot].lifespan		= atoi (row[4]);
		if (items[slot].lifespan == 100)
			items[slot].lifespan = 99;
		items[slot].count			= atoi (row[6]);
		items[slot].stats			= atoi (row[7]);
		items[slot].socketed		= (atoi (row[8]) == 1)?true:false;
		items[slot].appraised		= (atoi (row[9]) == 1)?true:false;
		items[slot].gem				= atoi (row[10]) > 3999? 0: atoi(row[10]);
		items[slot].sp_value		= atoi (row[11]);
		items[slot].restriction		= atoi (row[12]);		//PY added restriction 2022
		items[slot].usetype			= atoi (row[13]);		//PY added type 2022
		// Code to catch and convert older items that do not include valid values for the new fields
		CItem* TestItem = new (nothrow) CItem;								//cast a temporary CItem object
		TestItem = TestItem->GetCItem(items[slot].itemtype, items[slot].itemnum);		//grab values from the STBs
		items[slot].restriction = TestItem->restriction;
		items[slot].itemgrade = TestItem->itemgrade;
		items[slot].usetype = TestItem->usetype;
		//end catch and fix code

		//LMA: checking the count too :(
        if((items[slot].itemtype < 10 || items[slot].itemtype > 12) && items[slot].count != 1)
        {
            Log(MSG_WARNING,"%s:: Wrong item quantity in slot %i, %u*(%u::%u), reseting it to 1.",CharInfo->charname, slot,items[slot].count,items[slot].itemtype,items[slot].itemnum);
            items[slot].count=1;
        }

		CalculateSignature(slot);  //Calculate signature.

        //LMA: up to refine 15 now (2010/05)...
        switch (items[slot].refine)
        {
            case 0:
            case 16:
            case 32:
            ...
            default:
            {
                Log(MSG_WARNING,"Invalid refine %i for item (%i:%i) for %s",items[slot].refine,items[slot].itemtype,items[slot].itemnum,CharInfo->charname);
                items[slot].refine=0;
            }
            break;
	...
	while(row = mysql_fetch_row(result))
    {
		...
		UINT slot = atoi(row[5]);						//read the slot
		storageitems[slot].itemnum		= atoi(row[0]);
		storageitems[slot].itemtype	= atoi(row[1]);
		storageitems[slot].refine		= atoi(row[2]);
		storageitems[slot].durability	= atoi(row[3]);
		storageitems[slot].lifespan	= atoi(row[4]);
		if (storageitems[slot].lifespan == 100)
			storageitems[slot].lifespan = 99;
		storageitems[slot].count		= atoi(row[6]);
		storageitems[slot].stats		= atoi(row[7]);
		storageitems[slot].socketed	= (atoi(row[8]) == 1)?true:false;
		storageitems[slot].appraised	= (atoi(row[9]) == 1)?true:false;
		storageitems[slot].gem			= atoi(row[10]);
	}
	GServer->DB->QFree( );
    ...
    if(Clan->clanid!=0)
    {
    	result = GServer->DB->QStore("SELECT logo,back,name,grade,cp,money,skills FROM list_clan where id=%i", Clan->clanid);
        if(result==NULL) return false;
    	if(mysql_num_rows(result)!=1)
    	{
    	...
    	else
    	{
    	    ...
    	    Clan->CP= atoi(row[4]);
            Clan->money = (row[5] != NULL) ? strtoull(row[5], NULL, 10) : 0;
            memset(Clan->skills, 0, sizeof(Clan->skills));
            if (row[6] != NULL && strlen(row[6]) > 0)
            {
                char skills_buf[512];
                strncpy(skills_buf, row[6], sizeof(skills_buf) - 1);
                skills_buf[sizeof(skills_buf) - 1] = '\0';
                char* token = strtok(skills_buf, ",");
                int s_idx = 0;
                while (token != NULL && s_idx < 60)
                {
                    Clan->skills[s_idx++] = (short)atoi(token);
                    token = strtok(NULL, ",");
                }
            }
        }
    ...
    if(result!=NULL) //return false;
    {
        while(row = mysql_fetch_row(result))
        {
           ...
           unsigned long len = lengths ? lengths[0] : 0;
           if (len > sizeof(SQuestData)) len = sizeof(SQuestData);
           if( row[0] != NULL && len > 0 ) memcpy(&quest, row[0], len);
        }
    }
    GServer->DB->QFree( );
	   ...
	   if(Stats->MP < 0)
    	...
    	for(UINT i=0;i<MAX_INVENTORY;i++)
        {
            ...
            GServer->DB->QExecute("INSERT INTO items (owner, slotnum, itemnum, itemtype, refine, durability, lifespan, count, stats, socketed, appraised, gem, sp_value, restriction, usetype) VALUES(%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i) ON DUPLICATE KEY UPDATE owner=VALUES(owner),itemnum=VALUES(itemnum),itemtype=VALUES(itemtype),refine=VALUES(refine),durability=VALUES(durability),lifespan=VALUES(lifespan),slotnum=VALUES(slotnum),count=VALUES(count),stats=VALUES(stats),socketed=VALUES(socketed),appraised=VALUES(appraised),gem=VALUES(gem),sp_value=VALUES(sp_value)",
    								...
    								(items[i].appraised?1:0), items[i].gem, sp_item_value, items[i].restriction, items[i].usetype );
    	}
...
void CPlayer::DP( )
{
    if ( is_bot || Session->userid == 0 ) return;
    lastDpTime = clock();
    int DP = 0;
    int Counter = 0;
    clock_t mayGive;
...
void CPlayer::saveskills( )
{
    ...
    for(UINT i=0;i<MAX_CLASS_SKILL;i++)
    {
        ...
        //LMA: % dealer rebate and summon gauge and craft talent.
        if (cskills[i].thisskill!=NULL)
        {
            for(int j=0;j<3;j++)
            {
    ...
    discount = pc_temp;
    pc_up = pc_union;
    summon_gauge=50+val_summon;
    Log(MSG_INFO,"%s:: New gauge is %i",CharInfo->charname,summon_gauge);

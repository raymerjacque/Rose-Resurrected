...
class CMonster;				// Forward declare
class CPlayerBot;
...
class CPlayer: public CCharacter
{
   ...
   //LMA: last shout.
    ...
    struct SDelayedQuestTrigger {
        dword hash;
        time_t executeTime;
    };
    std::vector<SDelayedQuestTrigger> delayedTriggers;
    ...
    //PY Dealer skill merchandising. get more when selling to NPC
    int merchandising;

    // % for dealer (when buying from NPC).
    int discount;
	...
	void onCheckForMonsterQuests_event (CPlayer* player, CMonster* Monster);					// Called if the character had first hit and monster was killed.
    ...
    void ApplyClanPassive(UINT buffType1, UINT buffType2, UINT buffType3, UINT &pVal, UINT &vVal);
    ...
    void CheckDelayedQuestTriggers();

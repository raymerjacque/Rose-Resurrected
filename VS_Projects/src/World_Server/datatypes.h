...
// PY set this to use party level calculations from the 137 client
#define USE_137_PARTY_EXP
...
struct CEquip
{
    ...
    UINT attackpower;
    UINT attackspeed;
	UINT refineitem;	//PY points to refine recipe row in LIST_PRODUCT
...
struct CUseData
{
    ...
    UINT craft_recipe; 
    UINT craft_difficult; 
    //UINT usecondition[2];		//PY Totally wrong
    //UINT useeffect[2];
	UINT required_Stat;			//Using these 4 items instead of the two wrong arrays
	UINT required_Amount;
	UINT affected_Stat;
	UINT affected_Amount;
	UINT skill_number;
    UINT pricevalue;
...
struct CNPCData {
    ...
    unsigned long long exp;
  	...
  	UINT specialtab;
    float atkdistance;
    //UINT aggresive;												//PY removed these two because they are stupid
...
{
	/// Private Methods:
	void			_Init ();							// Internal Initialisation method.
	void			_Refresh ();
	ItemFamily	    _getFamily ();
...
public:
	...
	/// Public Constants
	static const int MAX_STACK_SIZE = 999;				// Since this 999 value applies to items, just define it here.

	/// FK Enum class would require casting. Regular Enum doesn't.
	enum InventoryTab {
		EquipmentTab = 0,
		ConsumableTab = 1,
		MaterialsTab = 2,
	...
	/// other properties:
	InventoryTab    InventoryTabID;
	ItemCategory	Category;
	ItemFamily		Family;									// Returns the general family for this item;

	/// Property Method Interface:

	bool		    isStackable () const;					// Indicator if the item is stackable.
	bool		    isEssentialGoods () const;				// Indicator if the item is essential (food, potions, craft mats etc. Used in Buy/Sell Formula's)
	bool			isValid () const;						// Indicates if item is valid based on a couple simple checks.
	bool			isAutoConsume () const;					// Indicates if item is of AutoConsume Type;

	/// Regular Method Interface;
	InventoryTab	GetInventoryTab ();

	void			Decrement (int count = 1);					// Decrease the item count with 1 or more. (Validated use)
	void			Increment (int count = 1);					// Increase the item count with 1 or more. (Validated use)

	CItem*			GetItemFromID (uint32 itemId);
	CItem*			GetCItem (BYTE itemtype, DWORD itemid );
	

	/// Constructors:
	CItem () {
		/// Calling the internal reset function;
		this->_initphase = true;
		this->_Init ();
		this->_initphase = false;

		if (this->_itemtype != eItemType::Money) {
			this->InventoryTabID = InventoryTab::UnknownTab;
			this->Stackable = false;
			this->Category = ItemCategory::Unknown;
			this->Family = ItemFamily::Unknown;
		}
	};
...
struct CNPC {
	...
	bool	canRepair = false;						// FK: Added to be read directly from the database. This elimiates the 'blacklist' in pakRepairItem
	uint32  id;										// PY: Added so that each NPC actually has a unique identifier that can be loaded in from the database. For use in addressing NPCs via GM commands
	std::string name = "";
	bool	active = "false";						// PY: Added so that some NPCs can be turned on or off either on load or during server operations
};

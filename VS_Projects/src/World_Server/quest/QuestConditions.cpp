...
QUESTCOND(001)
{
	...
	{
		uintptr_t address = i * sizeof(STR_QUEST_DATA);
...
QUESTCOND(004)
{
	...
	for(int i = 0; i < data->iDataCnt; i++)
	{
		uintptr_t address = i * 0x10;
		address += (uintptr_t)data;
		address += 4;
		STR_ITEM_DATA* curItem = (STR_ITEM_DATA*)address;

        if( client->questdebug )
            Log(MSG_QUESTDEBUG, "QUESTCOND(004):: Check Item - [Slot:%i][Type:%i][ReqCnt:%i][Op:%i]", curItem->iWhere, curItem->uiItemSN, curItem->iRequestCnt, curItem->btOp);
        ...
        {
            // Quest item check
            int j = client->CheckQuest;
            if (j < 0 || j >= 10)
            {
                if (client->ActiveQuest > 0)
                {
                    for (int q = 0; q < 10; q++)
                    {
                        if (client->quest.quests[q].QuestID == client->ActiveQuest)
                        {
                            j = q;
                            break;
                        }
            ...
            if (j >= 0 && j < 10)
            {
                ...
                {
                    if (client->quest.quests[j].Items[k].itemnum == ch_itemnum)
                    {
                        count += client->quest.quests[j].Items[k].count;
                    }
                }
            }
            else
            {
                // Fallback: search across all active quests
                for (int q = 0; q < 10; q++)
                {
                    for (unsigned int k = 0; k < 6; k++)
                    {
                        if (client->quest.quests[q].Items[k].itemnum == ch_itemnum)
                        {
                            count += client->quest.quests[q].Items[k].count;
                        }
                    }
                }
            }
            ...
            if( client->questdebug )
                Log(MSG_QUESTDEBUG, "QUESTCOND(004):: Quest Item Op: %i Val1: %i Val2: %i", curItem->btOp, count, curItem->iRequestCnt);

            if (!OperateValues<int>(curItem->btOp, &count, curItem->iRequestCnt))
            {
                return QUEST_FAILURE;
            }
        }
        ...
        {
            // Inventory item: sum counts across all 140 inventory slots
            int total_count = 0;
            for (int k = 0; k < 140; k++)
            {
                if (client->items[k].itemtype == ch_itemtype && client->items[k].itemnum == ch_itemnum)
                {
                    total_count += client->items[k].count;
                }
            }

            if( client->questdebug )
                Log(MSG_QUESTDEBUG, "QUESTCOND(004):: Inv Item Op: %i Val1: %i Val2: %i", curItem->btOp, total_count, curItem->iRequestCnt);

            if (!OperateValues<int>(curItem->btOp, &total_count, curItem->iRequestCnt))
            {
                return QUEST_FAILURE;
            }
        }
        else if (curItem->uiItemSN == 0 && curItem->iWhere != 0)
        {
            // Check equipped items for a specific item
...
QUESTCOND(012)
{
    ...
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(012):: Select IFO Object %i in zone %i (%i, %i)", data->iEventID, data->iZone, data->iX, data->iY);

    if(GServer->WarpGate.id == data->iEventID && GServer->WarpGate.mapid == data->iZone)
    {
        client->quest.RefNPC = GServer->WarpGate.virtualNpctypeID;
        return QUEST_SUCCESS;
    }

    if(data->iZone > 0 && data->iZone < GServer->MapList.max)
    {
        CMap* map = GServer->MapList.Index[data->iZone];
        if(map != NULL)
        {
            for(UINT i = 0; i < map->NPCList.size(); i++)
            {
                CNPC* npc = map->NPCList[i];
                if(npc != NULL && ((int)npc->npctype == data->iEventID || (int)npc->event == data->iEventID))
                {
                    client->quest.RefNPC = npc->npctype;
                    return QUEST_SUCCESS;
                }
            }
        }
...
QUESTCOND(016)
{
    GETCONDDATA(016);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(016):: Check Zone Time %u to %u", data->ulTime, data->ulEndTime);

    if(client->Position->Map == 0 || client->Position->Map >= GServer->MapList.max)
        return QUEST_FAILURE;

    CMap* map = GServer->MapList.Index[client->Position->Map];
    if(map == NULL) return QUEST_FAILURE;

    if(map->ZoneTime >= data->ulTime && map->ZoneTime <= data->ulEndTime)
        return QUEST_SUCCESS;
...
QUESTCOND(022)
{
    GETCONDDATA(022);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(022):: Check Server channel %u to %u", data->nX, data->nY);
    if(data->nX == 0 && data->nY == 0)
        return QUEST_SUCCESS;
    if(GServer->Config.ServerID >= data->nX && GServer->Config.ServerID <= data->nY)
        return QUEST_SUCCESS;
    return QUEST_FAILURE;
}
...
QUESTCOND(025)
{
    GETCONDDATA(025);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(025):: Check Clan Contribution (expected: %d, op: %d)", data->nCONT, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 )
        return QUEST_FAILURE;

    int cp = (int)client->Clan->CP;
    if(!OperateValues<int>(data->btOP, &cp, (int)data->nCONT))
        return QUEST_FAILURE;
...
QUESTCOND(028)
{
    GETCONDDATA(028);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(028):: Check Clan Funds (expected %u, op %u)", data->iMONEY, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 )
        return QUEST_FAILURE;

    int clan_money = (int)(client->Clan->money > 0x7FFFFFFF ? 0x7FFFFFFF : client->Clan->money);
    if(!OperateValues<int>(data->btOP, &clan_money, (int)data->iMONEY))
        return QUEST_FAILURE;
...
QUESTCOND(029)
{
    GETCONDDATA(029);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(029):: Check Clan Members (expected %u, op %u)", data->nMemberCNT, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 )
        return QUEST_FAILURE;

    MYSQL_RES *result = GServer->DB->QStore("SELECT COUNT(*) FROM characters WHERE clanid=%u", client->Clan->clanid);
    if(result == NULL)
        return QUEST_FAILURE;
    MYSQL_ROW row = mysql_fetch_row(result);
    int memberCount = row ? atoi(row[0]) : 0;
    GServer->DB->QFree();

    if(!OperateValues<int>(data->btOP, &memberCount, data->nMemberCNT))
        return QUEST_FAILURE;
...
QUESTCOND(030)
{
    GETCONDDATA(030);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTCOND(030):: Check Clan Skills (skill1: %u, skill2: %u, op: %u)", data->nSkill1, data->nSkill2, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 )
        return QUEST_FAILURE;

    bool hasSkill1 = false;
    bool hasSkill2 = (data->nSkill2 == 0);

    for(int s=0; s<60; s++)
    {
        if(client->Clan->skills[s] == 0) continue;
        if(data->nSkill1 != 0 && client->Clan->skills[s] == data->nSkill1)
            hasSkill1 = true;
        if(data->nSkill2 != 0 && client->Clan->skills[s] == data->nSkill2)
            hasSkill2 = true;
    }

    bool hasSkills = (data->nSkill1 == 0 || hasSkill1) && hasSkill2;
    int hasSkillsVal = hasSkills ? 1 : 0;
    int expected = 1;
    if(!OperateValues<int>(data->btOP, &hasSkillsVal, expected))
        return QUEST_FAILURE;

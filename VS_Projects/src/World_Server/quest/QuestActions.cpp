...
QUESTREWD(001)
{
    ...
    else
    {
		...
		Log (MSG_DEBUG, "[Quest Actions] reward given");
    }
...
QUESTREWD(010){
    ...
    ADDWORD( pak, client->clientid );
    ADDWORD( pak, client->Stats->Level );
    ADDDWORD( pak, client->CharInfo->Exp );
    ADDWORD( pak, client->CharInfo->StatPoints );
    ADDWORD( pak, client->CharInfo->SkillPoints );
    client->client->SendPacket( &pak );
...
QUESTREWD(012){
    GETREWDDATA(012);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(012):: NPC Speak/Shout StrID: %u, MsgType: %u", data->iStrID, data->btMsgType);

    if(data->iStrID >= (dword)GServer->maxltbqsd || GServer->LtbstringQSD[data->iStrID] == NULL)
        return QUEST_SUCCESS;

    int good_npc = client->quest.RefNPC;
    int good_map = client->Position->Map;

    char* npcName = GServer->GetSTLMonsterNameByID(good_npc);
    if(npcName == NULL)
        npcName = GServer->LtbstringQSD[data->iStrID]->NPCname;

    char* sayStr = GServer->LtbstringQSD[data->iStrID]->LTBstring;
    if(sayStr == NULL)
        return QUEST_SUCCESS;
    ...
    {
        // Shout
        GServer->NPCShout(NULL, sayStr, npcName, good_map);
    }
    else if(data->btMsgType == 2)
    {
        // Announce
        GServer->NPCAnnounce(sayStr, npcName, good_map);
    }
...
QUESTREWD(013)
{
	...
	dword hash = MakeStrHash(tempName);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(013):: Execute Quest Trigger %s[%d] [%08x] with %u sec time delay", tempName, data->nNameLength, hash, data->iSec);
    int tempval = client->quest.RefNPC;
    client->quest.RefNPC = tempval;

    CPlayer::SDelayedQuestTrigger dt;
    dt.hash = hash;
    dt.executeTime = time(NULL) + data->iSec;
    client->delayedTriggers.push_back(dt);
...
QUESTREWD(023)
{
    ...
    if( client->Clan == NULL || client->Clan->clanid == 0 ) return QUEST_SUCCESS;
    for(UINT i=0;i<GServer->MapList.Map.size();i++)
    {
        ...
        {
        CPlayer* player = map->PlayerList.at(j);
        if( player->Clan == NULL || player->Clan->clanid != client->Clan->clanid ) continue;
...
QUESTREWD(024)
{
    GETREWDDATA(024);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(024):: Clan Money (amount: %d, op: %d)", data->iMONEY, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 ) return QUEST_SUCCESS;

    int amount = data->iMONEY;
    if(data->btOP == 7) // deduction
    {
        if((uint64_t)amount > client->Clan->money)
            client->Clan->money = 0;
        else
            client->Clan->money -= (uint64_t)amount;
    }
    else
    {
        client->Clan->money += (uint64_t)amount;
    ...
    GServer->DB->QExecute("UPDATE list_clan SET money=%llu WHERE id=%i", (unsigned long long)client->Clan->money, client->Clan->clanid);
	return QUEST_SUCCESS;
}
...
QUESTREWD(026)
{
    GETREWDDATA(026);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(026):: Clan Skill (skill: %d, op: %d)", data->nSkillNo, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 || data->nSkillNo == 0 ) return QUEST_SUCCESS;
    ...
    for(int s=0; s<60; s++)
    {
        if(client->Clan->skills[s] == (short)data->nSkillNo)
        {
            alreadyLearned = true;
            break;
        }
        if(client->Clan->skills[s] == 0 && emptySlot == -1)
        {
            emptySlot = s;
        }
    }

    if(!alreadyLearned && emptySlot != -1)
    {
        client->Clan->skills[emptySlot] = (short)data->nSkillNo;

        // Build comma-separated skills string
        char skills_str[512] = "";
        char tmp_num[16];
        bool first = true;
        for(int s=0; s<60; s++)
        {
            if(client->Clan->skills[s] != 0)
            {
                if(!first) strcat(skills_str, ",");
                sprintf(tmp_num, "%d", client->Clan->skills[s]);
                strcat(skills_str, tmp_num);
                first = false;
            }
        }

        GServer->DB->QExecute("UPDATE list_clan SET skills='%s' WHERE id=%i", skills_str, client->Clan->clanid);

        // Recalculate stats for all online members of this clan
        for(UINT i=0; i<GServer->MapList.Map.size(); i++)
        {
            CMap* map = GServer->MapList.Map.at(i);
            if(map->PlayerList.size() < 1) continue;
            for(UINT j=0; j<map->PlayerList.size(); j++)
            {
                CPlayer* player = map->PlayerList.at(j);
                if(player->Clan != NULL && player->Clan->clanid == client->Clan->clanid)
                {
                    memcpy(player->Clan->skills, client->Clan->skills, sizeof(player->Clan->skills));
                    player->UpdateValues();
                }
            }
        }
...
QUESTREWD(027)
{
    GETREWDDATA(027);
    if( client->questdebug )
        Log(MSG_QUESTDEBUG, "QUESTREWD(027):: Clan Contribution: %d (op: %d)", data->nCONT, data->btOP);
    if( client->Clan == NULL || client->Clan->clanid == 0 ) return QUEST_SUCCESS;
    if( data->nCONT > 0 )
        client->GiveCP(data->nCONT);
    return QUEST_SUCCESS;
}
...
QUESTREWD(030) //Reset skills
{
    ...
    for (int k = 0; k < 60; k++)
    {
        if (client->cskills[k].thisskill != NULL && client->cskills[k].thisskill->sp > 0)
        {
            sp_points += client->cskills[k].level;
        }

        client->cskills[k].id = 0;
        client->cskills[k].level = 1;
        client->cskills[k].thisskill = NULL;
    }

    client->saveskills();
    client->ResetSkillOffset();
    if (sp_points > 0)
    {
        client->CharInfo->SkillPoints += sp_points;
        BEGINPACKET( pak, 0x720 );
        ADDWORD( pak, 37 );

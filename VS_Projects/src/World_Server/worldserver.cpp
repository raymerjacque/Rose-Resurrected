...
void CWorldServer::ServerLoop( )
{
	...
	do
	{
		...
		if ( FD_ISSET( sock, &fds ) && ConnectedClients < 1024 )
        {
			...
			{
				if (Config.MaxConnections > 0 && ConnectedClients >= Config.MaxConnections)
				{
					Log( MSG_WARNING, "WorldServer capacity reached (%u/%u). Rejecting connection from %s", ConnectedClients, Config.MaxConnections, inet_ntoa( ClientInfo.sin_addr ) );
					close( NewSocket );
				}
				else if(!isBanned(&ClientInfo))
...
void CWorldServer::LoadCommandLevels( void )
{
    ...
    //Config.Command_Reborn = ConfigGetInt    ( "commands.ini", "reborn", 99 );  //Reborn by core

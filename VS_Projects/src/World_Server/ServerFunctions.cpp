    ...
    switch(thisskill->script)
    {
        case 1:
        {
            ...
            if(character->IsPlayer())
            {
                ...
                if (tempplayer->cur_gauge>=tempplayer->summon_gauge)
                {
                    Log(MSG_WARNING,"Player %s tries to summon %i but his gauge is full (%i>=%i)",tempplayer->CharInfo->charname,thisskill->svalue1,tempplayer->cur_gauge,tempplayer->summon_gauge);
                    return false;
                }

                if(((int)(GServer->NPCData[thisskill->svalue1]->tab1) + tempplayer->cur_gauge) > tempplayer->summon_gauge)
                {
                    Log(MSG_WARNING,"Player %s tries to summon %i but won't have enough gauge (%i+%u>%i)",tempplayer->CharInfo->charname,thisskill->svalue1,tempplayer->cur_gauge,GServer->NPCData[thisskill->svalue1]->tab1,tempplayer->summon_gauge);
                    return false;
                }
            ...
            if (thismonster!=NULL)
            {
                ...
                {
                    CPlayer* tempplayer=(CPlayer*) character;
                    tempplayer->cur_gauge+=GServer->NPCData[thisskill->svalue1]->tab1;
                    thismonster->owner_user_id=tempplayer->Session->userid;
                    Log(MSG_INFO,"new gauge value %i/%i",tempplayer->cur_gauge,tempplayer->summon_gauge);
                }
...
int CPlayer::ExecuteQuestTrigger(dword hash,bool send_packet, UINT index)
{
	...
	if (GServer->questdebug)
        Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: Trigger executing hash %u, index %u", hash, index);
    ...
    for (dword i = 0; i < trigger->ConditionCount; i++)
    {
	    ...
	    {
            if (!trigger->CheckNext || nexttrigger == NULL)
    ...
    {
        if (GServer->questdebug)
            Log(MSG_QUESTDEBUG, "ExecuteQuestTrigger:: Sending 0x730 for hash %u, index %i", hash, index);
...
{   //if droptype = 1 then it is a normal drop. if it is 2 then it is a potential side drop.
    ...
    if(dropchance < 20) dropchance = 20; //always a small chance of a drop even when the mob is more than 20 levels beneath your own
	...
	newdrop->item = new (nothrow) CItem;	// thisdrop->item is a CItem* pointer so we have to properly initialize it
	Clearitem (newdrop->item);
	newdrop->item = newdrop->item->GetCItem (tmpDrop->itemtype, tmpDrop->itemnum);	// get data for the base version of the item
    newdrop->type = 2;
	//Now that we have initialized the drop item we can populate the rest of it
	...
	if (randomNumber > 80)
	{
		newdrop->item->stats = (rand () % 299) + 1;
	}
	else
		newdrop->item->stats = 0;
	if (newdrop->item->stats != 0)
...
{   //if droptype = 1 then it is a normal drop. if it is 2 then it is a potential side drop.
	...
	newdrop->item = new (nothrow) CItem;	// thisdrop->item is a CItem* pointer so we have to properly initialize it
	Clearitem (newdrop->item);
	newdrop->item = newdrop->item->GetCItem (MyMonsterDrops.at (n).type, MyMonsterDrops.at (n).item);	// get data for the base version of the item
...
*/
//receive the specified skill
bool CWorldServer::GiveSkill (CPlayer* thisclient, UINT skill, bool takeSP)
{
	int b = 1;
	CSkills* thisskill = GetSkillByID (skill);
	if (thisskill == NULL)
		return true;		//even though we did not learn this skill we have to return true or else the process will stop
	...
	if (b == 1)									// b is still 1 so it passed the first round of checks
	{
		for (UINT i = 0; i < 3; i++)			// now we check required skills
		{
			if (thisskill->rskill[i] != 0)		// there IS a required skill
			{
				UINT rskill = thisclient->GetPlayerSkill (thisskill->rskill[i]);
				if (rskill == 0xffff)			// did not have required skill
					b = 3;
				else
				{								// did have required skill. better check required level
	...
	if (b == 1)									// still good so now we can actually learn the skill
	{
		...
		{
			//Checking if the skill isn't already learned.
...
UINT CWorldServer::GetJobCode ( UINT myclass )
{
	switch (myclass)
	{
	case 111:

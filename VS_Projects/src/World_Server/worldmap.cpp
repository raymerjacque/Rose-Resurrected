...
CMonster* CMap::AddMonster( UINT montype, fPoint position, UINT owner, CMDrops* MonsterDrop, CMDrops* MapDrop, UINT spawnid , bool GetDropData, bool is_tactic)
{
    ...
    //LMA: no agressivity in Santa's planetoid ;)			///PY NO! NO! fucking NO!!!. Aggression is controlled by AIP
...
bool CMap::DeleteMonster( CMonster* monster, bool clearobject, UINT i )
{
	...
	if (monster->Position == NULL) return false;		//PY attempt to stop it coming through here twice
    ...
    //LMA TEST
    bool lma_debug = true;
    if(monster->Position->respawn == 4589)				//PY Under some circumstances monster->Position seems to be returning an invalid value and crashing here
    {
       lma_debug = true;
    ...
    {       // PY. No need to waste time searching the monsterlist if we already have its position
        for(UINT i=0;i<MonsterList.size();i++)
        {

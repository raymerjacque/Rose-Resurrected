...
void CWorldServer::SendToVisible( CPacket* pak, CPlayer* thisclient, bool dothisclient )
{
    for(UINT j=0;j<ClientList.size();j++)
    {
        ...
        if( IsVisible(otherclient, thisclient) || IsVisible(thisclient, otherclient) )
            otherclient->client->SendPacket( pak );
    }
...
{
    if ( !thisclient || !otherclient ) return false;

    if ( thisclient->is_bot )
    {
        for ( unsigned j = 0; j < otherclient->VisiblePlayers.size( ); j++ )
        {
            if ( thisclient == otherclient->VisiblePlayers.at( j ) )
                return true;
        }
        return false;
    }
...
char* CWorldServer::GetSTLItemPrefix(int family,UINT idorg)
{
    ...
    if(family >= 1 && family <= 9)
    {
        ...
        id = EquipList[family].itemList.at(idorg)->STLPrefix;
    }
...
char* CWorldServer::GetSTLObjNameByID(UINT family, UINT idorg,bool comment)
{
    ...
    if(family >= 1 && family <= 9)
    {
        ...
        id = EquipList[family].itemList.at(idorg)->STLId;
    }
...
CUseInfo* CWorldServer::GetUseItemInfo(CPlayer* thisclient, unsigned int slot )
{
    ...
    */
    ...
    switch(type)
    {
        ...
        {
            useitem->usescript = 1;
            useitem->usetype		= UseList.Index[useitem->itemnum]->affected_Stat;
            useitem->usevalue		= UseList.Index[useitem->itemnum]->affected_Amount;
        ...
        case 313://Magic Item [scrolls/summons/fireworks/etc]
        {
			...
			{
				useitem->usescript = 7;
				useitem->usetype = UseList.Index[useitem->itemnum]->affected_Amount;
				useitem->usevalue = 0;
			}
			...
			{
				useitem->usescript = 0;
				useitem->usetype = UseList.Index[useitem->itemnum]->required_Stat; // don't really need this as it is zero
				useitem->usevalue = UseList.Index[useitem->itemnum]->affected_Amount;
			}
...
//                else if(useitem->itemnum==609){useitem->usevalue = 2047;} // Event Metal Moldie
                     ...
                     useitem->use_buff = UseList.Index[useitem->itemnum]->affected_Stat;
                }
...
//                else if(useitem->itemnum==944){useitem->usevalue = 172;} // Arua's Blessing
        ...
        case 314://Skill Book
        {
            ...
            if(useitem->itemnum==609)
            {
                 ...
                 useitem->usescript = 5;
                 useitem->usetype = UseList.Index[useitem->itemnum]->affected_Stat;
                 useitem->use_buff = UseList.Index[useitem->itemnum]->affected_Amount;
            }
        ...
        case 321://Time Coupon - New Way to check TC
        {
              ...
              switch (UseList.Index[useitem->itemnum]->affected_Amount)
              {
                    case 138: //LMA: Null exp.
                    {
                        Log(MSG_INFO,"Null Exp activated for %s",thisclient->CharInfo->charname);
                        thisclient->no_exp = true;
                        thisclient->timer_no_exp = time(NULL)+60*(UseList.Index[useitem->itemnum]->affected_Amount *10);     //Effective Time Duration
                    ...
                    case 96://Special Shop Coupon, Shop A/B/C, Moldie, snowman, ....
                    {
                        ...
                        thisclient->Shop->mil_shop_time=time(NULL)+60*(UseList.Index[useitem->itemnum]->affected_Amount *10);     //Effective Time Duration
                        BEGINPACKET( pak, 0x702 );
                    ...
                    case 134://Medal of Fortune or Sigil of Wealth etc.. Set bonusddrop to quality - 1
                    {
                        if (thisclient->timerddrop == 0)
                        {
                            ...
                            thisclient->timerddrop = time(NULL) + 60 * (UseList.Index[useitem->itemnum]->affected_Amount * 10);  //Effective Time Duration
                    ...
                    case 135://Medal of Excellence
                    {
                        if (thisclient->timerstatdrop == 0)
                        {
                            ...
                            thisclient->timerstatdrop=time(NULL)+60*(UseList.Index[useitem->itemnum]->affected_Amount *10);  //Effective time duration
                    ...
                    case 136://Medal of Retrieval
                    {
                        if (thisclient->timergraydrop == 0)
                        {
                            ...
                            thisclient->timergraydrop=time(NULL)+60*(UseList.Index[useitem->itemnum]->affected_Amount *10);  //Effective time duration
...
UINT CWorldServer::ReturnItemType( CPlayer* thisclient, int slot )
{
    ...
    if(thisclient->items[material].itemtype >= 1 && thisclient->items[material].itemtype <= 9)
    {
        ...
        type=EquipList[thisclient->items[material].itemtype].itemList.at (thisclient->items[material].itemnum)->type;
    }
...
bool CWorldServer::IsValidItem(UINT type, UINT id )
{
    ...
    if(type<10)
    {
        if(id >= EquipList[type].max)
            return false;
        if(EquipList[type].itemList.at (id)->id == 0)
            return false;
    }
    else
    {
...
UINT CWorldServer::GetBreakID(UINT itemnum,UINT itemtype)
{
    ...
    breakid = EquipList[itemtype].itemList.at (itemnum)->breakid;
...
uint16 CWorldServer::GetItemRefineProb (CItem* thisItem) 
{
	//PY gonna use the list_upgrade STB table
	if (thisItem == NULL)	  return 0;
	uint16 iRet = 0;
	uint16 UpgradeIndex = ((thisItem->refine) / 16) + 1;	// Item->refine is refine level * 16 so we have to divide by 16 to get it to the right index value

	
	if (thisItem->itemtype == 8)		//It's a weapon so use talisman chance
		iRet = UpGradeList[UpgradeIndex].TaliSuccess;		//+1 because it's the level that we want to go to. 1 point higher than present level
	else
		iRet = UpGradeList[UpgradeIndex].BindSuccess;

	return iRet;
}

uint32 CWorldServer::GetItemRefinePrice (CItem* thisItem) {
	WORD currRefine = 0;
	DWORD nCost = 0;
	WORD Quality = 0;

	if (!thisItem) return 0;
	//CGameItem* gItem = GameData->Item (thisItem);

	if (!thisItem->isValid ()) {
		Log (MSG_HACK, "Invalid Item");
		return 0;
	}

	currRefine = thisItem->refine; if (currRefine >= 16) currRefine = currRefine / 16;
	Quality = thisItem->quality;

	/// From Client; return (int)( iGrade+1 * (iGrade + 2) * ITEM_QUALITY( GetTYPE(), GetItemNO() )  * (ITEM_QUALITY(GetTYPE(), GetItemNO()) + 20 ) * 0.2f );
	nCost = (DWORD)(currRefine) * (currRefine + 1) * Quality * (Quality + 20) * 0.20f;
	int a = 0;
	return nCost;
}

// Check if a slot is valid (to prevent crash)
bool CWorldServer::isValidSlot (CPlayer* thisclient, int slot) {
	if (slot >= MAX_INVENTORY || slot < 0) {
		Log (MSG_HACK, "Invalid Inventory slot %i from %s", slot, thisclient->Session->username);
		return false;
	}
...
CItem* CCraftRecipe::GetItemFromID (uint32 itemId)
{
	uint16 itemType = 0;
	uint16 itemNum = 0;
	CItem* thisItem = NULL;
...
CItem* CItem::GetCItem (BYTE itemtype, DWORD itemid)
{
	//method by which we can take any existing item in any of the other item lists and convert it to the relevent CItem
	CItem* thisitem = new (nothrow) CItem;
	thisitem->count = 1;		//We always return 1 from here since this is reading directly from the STB data. If you want the item count you will have to get it from player->items[]
	switch (itemtype)
	{
	case 1:		//mask
	case 2:		//hat
	...
	case 9:		//subweapon
	{
		CEquip* tempItem = new (nothrow) CEquip;
		tempItem = GServer->EquipList[itemtype].itemList.at (itemid);
		thisitem->craftskill = tempItem->craft_skill;
		thisitem->craftlevel = tempItem->craft_level;
		thisitem->craftdiff = tempItem->craft_difficult;
		thisitem->craftrecipe = tempItem->craft_recipe;
		thisitem->usetype = tempItem->type;
		thisitem->quality = tempItem->quality;
		thisitem->breakid = tempItem->breakid;
		thisitem->restriction = tempItem->restriction;
		thisitem->dodge = tempItem->dodge;
		thisitem->durability = tempItem->durability;
		thisitem->refineitem = tempItem->refineitem;
		thisitem->itemgrade = tempItem->itemgrade;

		break;
	}
	case 10:	//Useitem
	{
		CUseData* tempItem = new (nothrow) CUseData;
		tempItem = GServer->UseList.Index[itemid];
		thisitem->craftskill = tempItem->craft_skill;
		thisitem->craftlevel = tempItem->craft_level;
		thisitem->craftdiff = tempItem->craft_difficult;
		thisitem->craftrecipe = tempItem->craft_recipe;
	...
	case 11:		//Gem
	{
		CJemData* tempItem = new (nothrow) CJemData;
		tempItem = GServer->JemList.Index[itemid];
		thisitem->craftskill = tempItem->craft_skill;
		thisitem->craftlevel = tempItem->craft_level;
		thisitem->craftdiff = tempItem->craft_difficult;
		thisitem->craftrecipe = tempItem->craft_recipe;
	...
	case 12:	//Natural
	{
		CNaturalData* tempItem = new (nothrow) CNaturalData;
		tempItem = GServer->NaturalList.Index[itemid];
		thisitem->craftskill = tempItem->craft_skill;
		thisitem->craftlevel = tempItem->craft_level;
		thisitem->craftdiff = tempItem->craft_difficult;
		thisitem->craftrecipe = tempItem->craft_recipe;
		thisitem->usetype = tempItem->type;
		thisitem->quality = tempItem->quality;
		thisitem->refine = tempItem->refine_bonus;
		thisitem->restriction = tempItem->restriction;
		break;
	}
	case 14:	//PAT (carts and castlegear)
		CPatData* tempItem = new (nothrow) CPatData;
		tempItem = GServer->PatList.Index[itemid];
		thisitem->craftskill = tempItem->craft_skill;
		thisitem->craftlevel = tempItem->craft_level;
		thisitem->craftdiff = tempItem->craft_difficult;
		thisitem->craftrecipe = tempItem->craft_recipe;
...
void CItem::Decrement (int amount) 
{
	/// Decrement the item count, it will be destroyed if count reaches 0;
...
void CItem::Increment (int amount) 
{
	/// Safely increment the item count.
	// FK: While making sure it will never exceed max StackCount (999) or 1 (if the item is not stackable.);
	if (this->isStackable ()) 
	{
		if (this->count + amount > this->MAX_STACK_SIZE) 
		{
			this->count = this->MAX_STACK_SIZE;
		}
		else 
		{
			this->count += amount;

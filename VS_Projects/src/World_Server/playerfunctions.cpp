   ...
   //Update or add a slot (kinky way).
	...
	if (items[slot].itemtype > 0)
    {
        ...
        res_mysql=GServer->DB->QExecuteUpdate("INSERT INTO items (owner, slotnum, itemnum, itemtype, refine, durability, lifespan, count, stats, socketed, appraised, gem, sp_value, restriction, usetype) VALUES(%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i) ON DUPLICATE KEY UPDATE owner=VALUES(owner),itemnum=VALUES(itemnum),itemtype=VALUES(itemtype),refine=VALUES(refine),durability=VALUES(durability),lifespan=VALUES(lifespan),slotnum=VALUES(slotnum),count=VALUES(count),stats=VALUES(stats),socketed=VALUES(socketed),appraised=VALUES(appraised),gem=VALUES(gem),sp_value=VALUES(sp_value)",
    								...
    								(items[slot].appraised?1:0), items[slot].gem, sp_item_value, items[slot].restriction, items[slot].usetype );
...
//LMA: saving slot in database (outdated)		//PY commented out because there are no function calls to this 
...
void CPlayer::TakeFuel(int add_fuel)
{
    ...
    //LMA: new way (STB)
    float total_fuel = (float) GServer->PatList.Index[items[135].itemnum]->gauge + (float) GServer->PatList.Index[items[136].itemnum]->gauge;   //maximum fuel.
...
{
         return GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->type;
}
...
void CPlayer::RebuildItemMall()
{
	...
	while(row = mysql_fetch_row(result))
    {
		...
		if (items[itemnum].lifespan == 100)
			items[itemnum].lifespan = 99;		//PY: 99 is the maximum or else reroll hammers don't work
...
bool CPlayer::CheckDoubleEquip()
{
    ...
    UINT weapontype = GServer->EquipList[WEAPON].itemList.at (items[7].itemnum)->type;
    switch(weapontype)
    {
        case TWO_HAND_SWORD:
        case SPEAR:
...
void CPlayer::CheckDelayedQuestTriggers()
{
    time_t now = time(NULL);

    // 1. Process delayed quest trigger queue
    if (!delayedTriggers.empty())
    {
        for (size_t i = 0; i < delayedTriggers.size(); )
        {
            if (now >= delayedTriggers[i].executeTime)
            {
                dword hash = delayedTriggers[i].hash;
                delayedTriggers.erase(delayedTriggers.begin() + i);
                if (GServer->questdebug)
                    Log(MSG_QUESTDEBUG, "CheckDelayedQuestTriggers:: Firing delayed quest trigger hash %08x for %s", hash, CharInfo->charname);
                ExecuteQuestTrigger(hash, true);
            }
            else
            {
                ++i;
    ...
    for (unsigned i = 0; i < 10; i++)
    {
        if (quest.quests[i].QuestID > 0 && GServer->STB_QUEST.rows[quest.quests[i].QuestID][1] > 0)
        {
            ...
            {
                quest.quests[i].StartTime = 1; // Mark as expired
                if (GServer->questdebug)
                    Log(MSG_QUESTDEBUG, "Timed quest %u for %s has expired!", quest.quests[i].QuestID, CharInfo->charname);
                SendQuestUpdate();
            }
        }
    }
...
bool CPlayer::CheckStats(int slot, int dest_slot)
{
    ...
    for(int k=0;k<2;k++)
    {
        ...
        if(k == 0)
        {
            cdt = GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->condition1[0];
            //int value=GServer->EquipList[items[slot].itemtype].Index[items[slot].itemnum]->stat1[k];
            value = GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->condition1[1];
        }
        else
        {
            cdt = GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->condition2[0];
            //int value=GServer->EquipList[items[slot].itemtype].Index[items[slot].itemnum]->stat1[k];
            value = GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->condition2[1];
        }
    ...
    for (int k = 0;k<3;k++)
    {
        job_id = GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->occupation[k];
        if (job_id == 0)
        {
            nb_0++;
            continue;
    ...
    if(!is_ok)
    {
        Log(MSG_HACK,"Player %s tried to equip item %u::%u slot %i but job check failed (idx %i, %i, %i (see list_class.stb), job %i)", CharInfo->charname, items[slot].itemtype, items[slot].itemnum,slot,
            GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->occupation[0],
            GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->occupation[1],
            GServer->EquipList[items[slot].itemtype].itemList.at (items[slot].itemnum)->occupation[2],
            CharInfo->Job);
        return false;
    }

...
void CCharacter::DoAttack( )
{
	...
	if(Battle->skillid > 0)
		skill = GServer->GetSkillByID (Battle->skillid);
	//if (Enemy == NULL)
	//	return;						//well this is dumb. Any skill without an enemy will just be ignored while this is happening
    if(IsSummon())
    {
	...
	case NORMAL_ATTACK://normal attack
                    ...
                    if (Enemy->IsMonster()) // do monster AI script when monster is attacked. NO TIMER here
                    {
                        ...
                        if(monster != NULL)
                        {
                            if(!monster->IsDead())
                            {
                                ...
                                if(monster->thisnpc != NULL && !monster->TD) //PY no AI stuff for TD monsters
        ...
        case SKILL_ATTACK://skill attack
        {
            ...
            if(IsTargetReached( Enemy, skill ))
            {
                ...
                if(CanAttack( ))
                {
                    bool isStartingCast = (Battle->castTime == 0);
                    if (isStartingCast)
                    {
				...
				if(isStartingCast && skill->aoerange>0)
                {
                ...
                if (isStartingCast && Enemy->IsMonster())
                {
                    CMonster* monster = GServer->GetMonsterByID(Enemy->clientid, Enemy->Position->Map);

                    if(monster!=NULL && monster->thisnpc!=NULL && !monster->IsDead() && !monster->TD) //PY No AI stuff for TD monsters
                        monster->DoAi(monster->thisnpc->AI, 3);
                }
...
void CCharacter::NormalAttack( CCharacter* Enemy )
{
	...
	long int hitpower = 0;
	bool critical = false;
	float MyAccuraccy = Stats->Accuracy ? (float)Stats->Accuracy : 1.0f;
	float EnemyDodge = Enemy->Stats->Dodge ? (float)Enemy->Stats->Dodge : 1.0f;
	float MyAttack = Stats->Attack_Power ? (float)Stats->Attack_Power : 1.0f;
	float EnemyDef = Enemy->Stats->Defense ? (float)Enemy->Stats->Defense : 1.0f;
	float EnemyMDef = Enemy->Stats->Magic_Defense ? (float)Enemy->Stats->Magic_Defense : 1.0f;

	//hit chance based on accuracy vs dodge
	hitchance = (MyAccuraccy / EnemyDodge) * 50.0f;
	if (hitchance > 95.0f)
		hitchance = 95.0f;
	if (hitchance < 5.0f)
		hitchance = 5.0f;

	//Let's see if we successfully hit or not
	...
	{			//Success. Now we check Attack Power against Defence
		...
		if(IsPlayer( )) //temp fix to find balance btw monster and player
			hitpower = (long int)floor(hitpower * (GServer->Config.PlayerDmg/100.00));
		if(IsMonster( )) //temp fix to find balance btw monster and player
			hitpower = (long int)floor(hitpower * (GServer->Config.MonsterDmg/100.00));

		if (hitpower < 1)
			hitpower = 1;

		if(IsPlayer() && hitpower > 0)//Add Dmg
		{
		...
		//Lets check for critical chance
		...
		float HitMod = (float)hitpower / 10.0f;
		if (HitMod >= 1.0f)
		{
			int MyRand = (GServer->RandNumber (0, (int)(HitMod * 2.0f))) - (int)HitMod;		//range of 90% to 110%
			hitpower += MyRand;
		}
		// And once more make sure that we didn't dip below 1 damage
		if (hitpower < 1)
			hitpower = 1;

		Log(MSG_INFO,"Combat: %s %u hit %s %u: dmg=%li, crit=%d, roll=%.1f/%.1f",
			IsPlayer() ? "Player" : "Monster", clientid,
			Enemy->IsPlayer() ? "Player" : "Monster", Enemy->clientid,
			hitpower, critical ? 1 : 0, DidWeHit, hitchance);
	}
	else
	{
		Log(MSG_INFO,"Combat: %s %u missed %s %u: roll=%.1f/%.1f",
			IsPlayer() ? "Player" : "Monster", clientid,
			Enemy->IsPlayer() ? "Player" : "Monster", Enemy->clientid,
			DidWeHit, hitchance);
	}
	//if hitpower is still zero then this will not do anything.
    
    if(hitpower <= 0)	//Just making sure that hitpower didn't end up negative because that would be bad. 
    {
        hitpower = 0;
    }
   
    if(!Enemy->IsSummon( ) && Enemy->IsMonster( ) && hitpower > 0)	//This funtion used to track damage done to a monster for exp purposes and for AIP
    {
        Enemy->AddDamage( this, (long long) hitpower );				// NOTE: No actual damage is done to the monster here.
	...
	//Damage reflect
    if(hitpower > 0 && Enemy->Status->ShieldDamage_up != 0xff && ((Stats->HP) > ((long long) (hitpower*(Enemy->shield_reflect) / 100))))
    {
...
bool CCharacter::ResumeNormalAttack( CCharacter* Enemy,bool was_aoe)
{
    ...
    if (IsPlayer())
    {
        ...
        // Check if we should continue attacking the target
        if (Battle->contatk && (Battle->target != 0 || Battle->skilltarget != 0 || Battle->atktarget != 0))
        {
            UINT tid = Battle->target ? Battle->target : (Battle->skilltarget ? Battle->skilltarget : Battle->atktarget);
            CCharacter* NAEnemy = NULL;
            if (Enemy != NULL && (Enemy->clientid == tid || Enemy->clientid == Battle->atktarget))
            {
                NAEnemy = Enemy;
            }
            else
            {
                CMap* map = GServer->MapList.Index[Position->Map];
                NAEnemy = map ? map->GetCharInMap( tid ) : NULL;
            }

            if (NAEnemy == NULL || NAEnemy->IsDead())
            {
                ClearBattle(Battle);
                return true;
            }
            ...
            Battle->bufftarget = 0;
            Battle->skillid = 0;
            StartAction(NAEnemy, NORMAL_ATTACK, 0);
            return true;
        }
...
bool CCharacter::AoeSkill( CSkills* skill, CCharacter* Enemy )
{
    ...
    if(IsPlayer() || IsSummon())
    {
		...
		{				//PY about the only option not covered is when the Enemy is actually NULL such as when it is a self centered AOE Buff skill like Blessed Mind
			if (Battle->atktype == AOE_TARGET && skill->target == 1)		///PY AOE effecting ONLY party members and self
			{
				CPlayer* thisPlayer = (CPlayer*)(this);
				for (UINT i = 0; i < map->PlayerList.size (); i++)
				{
					CPlayer* player = map->PlayerList.at (i);
					if (GServer->IsPlayerInCircle (thisPlayer, player, (float)skill->aoeradius + 1))
					{
						if (GServer->ServerDebug)
							Log (MSG_INFO, "AOE BUFF party members around player %s radius %.2f", player->CharInfo->charname, (float)skill->aoeradius + 1);
						UseBuffSkill ((CCharacter*)player, skill);
					}

				}
			}
			if (Battle->atktype = SKILL_AOE)
			{
				int TargetCount = 0;
				for (UINT i = 0; i < map->MonsterList.size (); i++)
				{
				...
				if (TargetCount == 0)			//PY: if we didn't actually hit any targets then we need to send the animation packet anyway. Otherwise it hangs up.
				{ 
...
void CCharacter::UseAtkSkill( CCharacter* Enemy, CSkills* skill, bool deBuff )
{
    ...
    float elevel = Enemy->Stats->Level ? (float)Enemy->Stats->Level : 1.0f;
    float levelmult = (float) Stats->Level / elevel;
    ...
    float edodge = Enemy->Stats->Dodge ? (float)Enemy->Stats->Dodge : 1.0f;
    float edef = Enemy->Stats->Defense ? (float)Enemy->Stats->Defense : 1.0f;
    float emdef = Enemy->Stats->Magic_Defense ? (float)Enemy->Stats->Magic_Defense : 1.0f;

    unsigned int skill_hitvalue = (unsigned int)floor(Stats->Accuracy * 50.0f / edodge);
    if(IsMonster( ) && !IsSummon())
        skill_hitvalue = (unsigned int)floor(Stats->Accuracy * 60.0f / edodge);
    ...
    switch(skill->formula)//Magical Or Weapon Type Skill?
    {
        ...
        {
            atkdefmult = (float) Stats->Attack_Power / (edef / 1.3f);
            skillpower += GetSen();
        ...
        {
            atkdefmult = (float) Stats->Attack_Power / (emdef / 1.3f);
            skillpower += GetInt();
            if(GServer->RandNumber( 0, 100 )>skill_hitvalue) atkdefmult /= 2; // Miss results in damage cut in half.
        }
        break;
...
void CCharacter::QuestKill(dword die_quest)
{
    ...
    plkiller->arr_questid[oldest].die_time=mytime+(CLOCKS_PER_SEC*10);
    if(GServer->ServerDebug)

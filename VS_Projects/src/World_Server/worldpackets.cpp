...
bool CWorldServer::pakSpawnNPC( CPlayer* thisclient, CNPC* thisnpc )
{
    ...
    // TagADDChar struct which also applies to characters
	ADDWORD( pak, thisnpc->clientid );
	ADDFLOAT( pak, thisnpc->pos.x*100 );				//Current x
    ADDFLOAT( pak, thisnpc->pos.y*100 );				//Current y
    ADDFLOAT( pak, thisnpc->pos.x*100 );				//Destination x (if moving)
    ADDFLOAT( pak, thisnpc->pos.y*100 );				//Destination y (if moving)
	...
	ADDWORD( pak, 0x0000 );								//WORD Target Object (m_wTargetOBJ). interacting with a target
	ADDBYTE( pak, 0x00 );								//BYTE movemode: 0:Walk, 1 run, 2: ride in my passenger Item 3: Riding is riding on items m_wTargetOBJ.
	ADDWORD( pak, 0x03e8 );								// int HP
	ADDWORD( pak, 0x0000 );								// int MaxHP
	ADDWORD( pak, 0x0001 );								// int Team number
	ADDWORD( pak, 0x0000 );//Buffs						// DWORD m_dwStatusFALG. That accounts for 2 WORDS anyway
	ADDWORD( pak, 0x0000 );//Buffs
	ADDWORD( pak, 0x0000 );//buffs						// No idea why this exists. It really shouldn't be here unless the buffs have been extended in some way
	// struct gsv_MOB_CHAR
	ADDWORD( pak, thisnpc->npctype );					// short m_nCharIdx (monster type)
	// short m_nQuestIDX. I guess this is the dialogID
...
bool CWorldServer::pakDoDrop( CPlayer* thisclient, CPacket* P )
{
	...
	BYTE slot = GETBYTE((*P), 0x0);
	if(!CheckInventorySlot(thisclient, slot ))
	{
        return false;				// no available inventory item to drop
	}

	DWORD amount = GETDWORD((*P), 0x1);
	if (slot == 0)			// drop zulie
    {
		if ( amount < 1 )
		{
		    return true;
    ...
    {	// Drop an item
        // is there something to drop to begin with?
        if(thisclient->items[slot].itemtype == 0 || thisclient->items[slot].itemnum == 0)
        {
            Log(MSG_WARNING,"%s tries to drop empty item from slot %u",thisclient->CharInfo->charname, slot);
            return true;
        }

        // fixed by tomciaaa [item count drop correctly and dissaper from inventory]
        bool flag = false;
		//if(thisclient->items[slot].itemtype >= 10 && thisclient->items[slot].itemtype <= 13)	//
		if (thisclient->items[slot].IsStackable())		// can we stack the item? type 10, 11 or 12
        {
            if(thisclient->items[slot].count < amount)
            {
                Log(MSG_WARNING,"%s tries to %i*(%u::%u) slot %u but he has only %u",thisclient->CharInfo->charname,amount,thisclient->items[slot].itemtype,thisclient->items[slot].itemnum,slot,thisclient->items[slot].count);
                return true;
            }
			thisclient->items[slot].count -= amount;
			flag = (thisclient->items[slot].count <= 0);
        }
        else
        {
           flag= true;
		...
		thisdrop->item			= new (nothrow) CItem;	// thisdrop->item is a CItem* pointer so we have to properly initialize it
		Clearitem (thisdrop->item);
		thisdrop->item = thisdrop->item->GetCItem (thisclient->items[slot].itemtype, thisclient->items[slot].itemnum);	// get data for the base version of the item
		//thisdrop->item = &thisclient->items[slot];
		//assert(thisdrop);
		thisdrop->clientid		= GetNewClientID();
		thisdrop->pos			= RandInCircle( thisclient->Position->current, 3 );
		thisdrop->posMap		= thisclient->Position->Map;
		...
		if (!thisclient->items[slot].IsStackable ())
		{																		//has to be equipable to do this
			thisdrop->item->lifespan = thisclient->items[slot].lifespan;
			thisdrop->item->refine = thisclient->items[slot].refine;
			thisdrop->item->durability = thisclient->items[slot].durability;
			thisdrop->item->durabLeft = thisclient->items[slot].durabLeft;
			thisdrop->item->stats = thisclient->items[slot].stats;
			thisdrop->item->socketed = thisclient->items[slot].socketed;
		}
		thisdrop->owner = 0;
		CMap* map = MapList.Index[thisdrop->posMap];
		map->AddDrop( thisdrop );
    ...
    if( (thisdrop->owner==0 || thisdrop->owner==thisclient->CharInfo->charid || time(NULL)-thisdrop->droptime>=30 ) || ( thisclient->Party->party!=NULL && thisclient->Party->party == thisdrop->thisparty ))
    {
        ...
        if( thisdrop->type == 2 ) // Item
        {
            ...
            if (type == 320 && thisdrop->item->itemtype == 10)
            {
                ...
                switch (UseList.Index[thisdrop->item->itemnum]->affected_Stat)
                {
                       case 16:
                       {
                            dropowner->Stats->HP += (UseList.Index[thisdrop->item->itemnum]->affected_Amount);
                       ...
                       case 17:
                       {
                            dropowner->Stats->MP += (UseList.Index[thisdrop->item->itemnum]->affected_Amount);
                       ...
                       case 76:
                       {
                            dropowner->CharInfo->stamina += (UseList.Index[thisdrop->item->itemnum]->affected_Amount);
                       ...
                       case 92:
                       {
                            if(dropowner->Clan->clanid>0)
                            {
                                ...
                                UINT points=UseList.Index[thisdrop->item->itemnum]->affected_Amount;
...
bool CWorldServer::pakChangeEquip( CPlayer* thisclient, CPacket* P )
{
    if(thisclient->Shop->open)
        return true;
	if (thisclient->Trade->trade_status != 0 && Config.no_unequip_during_trade == 1)
		return true;
    ...
    if (destslot < 13 && GServer->EquipList[thisclient->items[srcslot].itemtype].itemList.at (thisclient->items[srcslot].itemnum)->level > thisclient->Stats->Level)
       return true;
    ...
    if( srcslot == 7 || destslot == 7 )
    {
        ...
        UINT weapontype = EquipList[WEAPON].itemList.at(thisclient->items[7].itemnum)->type;
        switch(weapontype)
        {
            case TWO_HAND_SWORD:
            case SPEAR:
...
bool CWorldServer::pakStartAttack( CPlayer* thisclient, CPacket* P )
{
	...
	//if(!thisclient->CanAttack( )) return true;		///PY This is wrong. An asynchronous thread has no business calling a battle function
...
bool CWorldServer::pakGate( CPlayer* thisclient, CPacket* P )
{
	...
	for (int k=0;k<6;k++)
	{
	    ...
	    if (thisgate == NULL)
	    {
			...
			//If the gate doesn't exist just kick out immediately
            return true;
	    }
...
#ifdef DISABLE_GATE_HACKING_PROTECTION		///PY Set this in worldserver.h and recompile if you want to disable gate hacking code below
	is_hack = false;						// force hack test to false
#endif
	...
	if(is_hack)
	{
        Log( MSG_HACK, "Player %s[Map: %i X: %f Y: %f] - Gate Hacking[ID: %i, offset %i], distance %.2f",
                     thisclient->CharInfo->charname, thisclient->Position->Map,
                     thisclient->Position->current.x, thisclient->Position->current.y, GateID,lastgate->offset,distance(thisclient->Position->current, lastgate->src) );
		//Set destination to current position (server position that is. If we got here anyway then server and client are different)
		map = thisclient->Position->Map;
		position = thisclient->Position->current;
	}
	else
	{ 
		//Set destination to wherever the warp portal goes
...
bool CWorldServer::pakNPCBuy ( CPlayer* thisclient, CPacket* P )
{
	...
	//LMA: % of rebate.
	float pc_reb = (100 - (float)thisclient->discount) / 100;
	if (pc_reb<=0)
	{
	    pc_reb = 1;
	}
        ...
        while (tmpcount>0)
        {
    		...
    		switch(thisitem.itemtype)
    		{
                ...
                case 14:
                {
                    ...
                    {
                        quality = EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum)->quality;
                        bprice = EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum)->price;
                    }
                    else
                    {
                    ...
                    if (is_clanshop)
                    {
                        if(thisitem.itemtype<10 )
                        {
                            price=EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum)->craft_difficult;
                        }
                        else
                        {
                ...
                case 12:
                {
                    ...
                    if(thisitem.itemtype==10)
                    {
                        ...
                        //pvalue = UseList.Index[thisitem.itemnum]->pricevalue;		//PY this is Bollox!!! there IS NO pricevalue in the STB table
                    }
                    else
                    {
                        ...
                        //pvalue = NaturalList.Index[thisitem.itemnum]->pricevalue;	//PY this is Bollox!!! there IS NO pricevalue in the STB table
                    }
                    ...
                    if(flag)
                    {
                        //value = pvalue;			//PY NOT happening as pvalue doesn't exist. Therefore value is always equal to zero
                        if(value >= 11)				//Never true
                            value ^= 0xffffffff;
                        else
                            value = values[value];	//so value is always going to be 0x32
                        value -= 0x32;				//And now we subtract 0x32 so it's fucking zero again
                        value *= pricerate;			//still zero
                        price = value * 0.001;		//and now price is also zero
                        price += 1;					//Oh look!! Now it's 1
                        price *= bprice;			//bprice is the basic price straight from the STB which means that price is now equal to bprice
                        price += 0.5;				//plus a half for some reason

                        //LMA: price tweak.
                        if(!is_clanshop && !is_union)
                        {
                            //price = (float)floor(price/1.25);
                            price = (float)floor(price * pc_reb);	//pc_reb has is based on dealer discount
                        }
                        else
                        {
                ...
                case 13:
                {
                    ...
                    {
                        pricerate = EquipList[7].itemList.at (thisitem.itemnum)->pricerate;
                        bprice = EquipList[7].itemList.at (thisitem.itemnum)->price;
                    }
                    else
                    {
                        switch(thisitem.itemtype)
                    ...
                    if (is_clanshop)
                    {
                        if(thisitem.itemtype == 7)
                        {
                            price = EquipList[7].itemList.at (thisitem.itemnum)->craft_difficult;
                        }
	...
	for (int i=0; i<sellcount; i++)
    {
		...
		switch(thisitem.itemtype)
		{
            ...
            case 14:
            {
                ...
                CEquip *thisEquip = EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum);
                uint32  itemBasePrice = 0;
                int buySkillLevel = 0;
                if(thisitem.itemtype < 10)
                {
                    //itemBasePrice *= EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum)->price;
                    itemBasePrice = thisEquip->price;
                }
                else
                {
                    itemBasePrice *= PatList.Index[thisitem.itemnum]->price;
                }
                price = itemBasePrice;
                ...
                if (thisitem.appraised)
                    price += (EquipList[7].itemList.at (thisitem.gem)->price * 0.2f);
                price = (float)floor(price);
                ...
                //PY formula from Omega
                /// Now we can make the formula. Use the same crappy formula composition as the client to reduce rounding errors
                int32   iWorldRate = 100;
                int32   iTownRate = 100;
                float32 currGemValue = 0.f;
                uint32 iPrice = 0;
                iPrice = (uint32)(
                    (float32)itemBasePrice *
                    (float32)(40 + thisitem.refine) *
                    (float32)(200 + thisitem.durability) *
                    (float32)(200 - iWorldRate) *
                    (float32)((4000 + thisitem.lifespan) / 14000.f)
                    ) +
                    currGemValue;

                iPrice = (uint32)floor (float32 (iPrice / 0.9847f) + 0.5f);
            ...
            case 13:
            {
                ...
                {
                    case 7:
                        pricerate = EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum)->pricerate;
                        bprice = EquipList[thisitem.itemtype].itemList.at (thisitem.itemnum)->price;
                    break;
                    case 11:
                        pricerate = JemList.Index[thisitem.itemnum]->pricerate;
...
{
    if( thisclient->Shop->open || thisclient->Status->Mute !=0xff || !thisclient->Status->CanCastSkill)
        return true;
    fPoint thispoint;
    ...
    bool isDriveSkill = (thisclient->GoodSkill(skillid) == 1 || (thisskill->skill_tab >= 30 && thisskill->skill_tab <= 39));
    if(thisclient->Status->Stance == DRIVING)
    {
        if(!isDriveSkill)
            return true;
        if(thisclient->items[136].itemnum == 0 || thisclient->items[136].lifespan == 0)
            return true;
    }
    else
    {
...
bool CWorldServer::pakGiveQuest( CPlayer* thisclient, CPacket* P )
{
  ...
  if (MapStackQuest.find(hash) != MapStackQuest.end())
  {
    ...
    for (int k=0;k<10;k++)
    {
        ...
        //checking the time now (up to 10 seconds between death packet and quest packet).
...
bool CWorldServer::pakSkillSelf( CPlayer* thisclient, CPacket* P )
{
	...
	if (isSkillTargetFriendly (thisskill)) 
	{
		if (thisskill->aoe == 1)		// it is an AOE buff
		{ 
			thisclient->StartAction (NULL, BUFF_AOE, skillid);
		}
		else
		{
			if (thisskill->script == 1)			//It is a summon skill
			{
				thisclient->StartAction (NULL, SUMMON_BUFF, skillid);
			}
			else
			{									//has to be a self buff
 ...
 //   else
 //   {
 //       if(isSkillTargetFriendly( thisskill ))					//PY: Who really cares? This also is controlled by the skill->type if you just write the code properly
 //       {
 //           thisclient->StartAction( NULL, BUFF_AOE, skillid );
 //       }
 //       else
...
bool CWorldServer::pakUseItem ( CPlayer* thisclient, CPacket* P )
{
	...
	bool flag = false;
	UINT ItemNum = thisclient->items[slot].itemnum;
	CUseData* useitem = new (nothrow) CUseData;			//PY CUseInfo method is dumber than a rock. Rewriting the whole damn function
	useitem = UseList.Index[ItemNum];
	if (useitem == NULL)
	{
		delete useitem;
	...
	// lets check for required conditions
	UINT myJob = GetJobCode (thisclient->CharInfo->Job);
	switch (useitem->required_Stat)
	{
		case 4:		//	Job
		{
			if (myJob != useitem->required_Amount)
				return false;
		}
		break;
		case 31:	// Level
		{
			if (thisclient->Stats->Level < useitem->required_Amount)
				return false;
		}
		break;
		case 75:	// Stamina
		{
			if (thisclient->CharInfo->stamina < useitem->required_Amount)
				return false;
		}
		break;
	}
	switch (useitem->type)
	{
		case 311:		/// potions
		case 312:		/// food
		case 313:		/// magic items (using skills) including some potions, grenades and return scrolls
		case 320:		/// instant use items
			
			if (useitem->affected_Stat != 0)		
			{											// in normal cases we have a stat and an amount so handle that here
				if (useitem->affected_Stat == 16)		//HP
				{
					thisclient->Stats->HP += useitem->affected_Amount;
					flag = true;
				}
				if (useitem->affected_Stat == 17)		//MP
				{
					thisclient->Stats->MP += useitem->affected_Amount;
					flag = true;
				}
				if (useitem->affected_Stat == 37)		//skill points (mana jam)
				{
					thisclient->CharInfo->SkillPoints += useitem->affected_Amount;
					BEGINPACKET (pak, 0x720);
					ADDWORD (pak, 37);
					ADDWORD (pak, useitem->affected_Amount);
					ADDWORD (pak, 0);
					thisclient->client->SendPacket (&pak);
					flag = true;
				}
				if (useitem->affected_Stat == 76)		//stamina
				{
					thisclient->CharInfo->stamina += useitem->affected_Amount;
					flag = true;
				}
				if (useitem->affected_Stat == 92)
				{
					thisclient->GiveCP (useitem->affected_Amount);
					flag = true;
				}
				BEGINPACKET		(pak, 0x7a3);
				ADDWORD			(pak, thisclient->clientid);
				ADDWORD			(pak, useitem->id);
				SendToVisible	(&pak, thisclient);
			}
			else
			{											//Whenever the affected stat is zero we run a skill instead
				CSkills* thisskill = GetSkillByID (useitem->skill_number);
				if (thisskill->type == 18)											///teleport (return scrolls). regular skill handler is too primitive to handle warp skills. Meh!!
				{
					fPoint thispoint = thisskill->WarpData;
					GServer->TeleportTo (thisclient, thisskill->WarpZone, thispoint);
					flag = true;
				}
				else
				{ 
					thisclient->StartAction (NULL, BUFF_SELF, useitem->skill_number);			//This includes stuff like fireworks and summon skills (grab bags or easter bunny capsules) 
					flag = true;
				}
			}
		...
		case 314:		/// Skillbooks
			{
				flag = LearnSkill (thisclient, useitem->skill_number);
				flag = true;
			}
		break;
		case 315:		/// repair hammer
			{
		...
		case 317:		///fuel
		{
			thisclient->TakeFuel (useitem->affected_Amount);
			flag = true;
		}
		break;
		case 318:		///drills
		...
		case 321:		///medals
		{
			if (useitem->affected_Stat == 95)//Storage Expansion Coupon
			{
			...
			if (useitem->affected_Stat == 96)		//Special Shop Coupon, Shop A/B/C, Moldie, snowman, ....
			{
				Log (MSG_INFO, "Shop changed to %i", useitem->quality);
				thisclient->Shop->ShopType = useitem->quality;
				thisclient->Shop->mil_shop_time = time (NULL) + 60 * (useitem->affected_Amount * 10);     //Effective Time Duration
				BEGINPACKET (pak, 0x702);
			...
			if (useitem->affected_Stat == 133)		//exp medal
			{
				if (thisclient->timerxp == 0)
				{
					thisclient->Stats->xprate *= useitem->quality / 100;			// in most cases quality / 100 will result in 2X
					thisclient->timerxp += useitem->affected_Amount * 10 * 60;
					thisclient->wait_validation = thisclient->timerxp;
					BEGINPACKET (pak, 0x702);
					char msg[256];
					if(useitem->affected_Amount * 10 * 60 <= 144)
					{
						sprintf_s (msg, "The effect will hold until you play for %i minutes.", useitem->affected_Amount * 10 * 60);
						ADDSTRING (pak, msg);
					}
					if (useitem->affected_Amount * 10 * 60 > 144)
					{
						sprintf_s (msg, "The effect will hold until you play for %i days.", (useitem->affected_Amount * 10 * 60) / 144);
						ADDSTRING (pak, msg);
					}
					ADDBYTE (pak, 0);
					thisclient->client->SendPacket (&pak);
				...
				else
				{
					...
					BEGINPACKET (pak, 0x702);
					ADDSTRING (pak, "Please wait until your current Exp boost expires.");
					ADDBYTE (pak, 0);
					thisclient->client->SendPacket (&pak);
					return false;
				}
			...
			if (useitem->affected_Stat == 134)			//Medal of Fortune or Sigil of Wealth etc.. Set bonusddrop to quality - 1
			{
				if (thisclient->timerddrop == 0)
				{
					thisclient->bonusddrop = useitem->quality - 1;
					thisclient->wait_validation_ddrop = useitem->quality - 1; //quality
					thisclient->timerddrop = time (NULL) + 60 * (useitem->affected_Amount * 10);  //Effective Time Duration
			...
			if (useitem->affected_Stat == 135)//Medal of Excellence
			{
				if (thisclient->timerstatdrop == 0)
				{
					thisclient->bonusstatdrop = 1;
					thisclient->wait_validation_statdrop = useitem->quality;
					thisclient->timerstatdrop = time (NULL) + 60 * (useitem->affected_Amount * 10);  //Effective time duration
			...
			if (useitem->affected_Stat == 136)		//Medal of Retrieval
			{
				if (thisclient->timergraydrop == 0)
				{
					thisclient->bonusgraydrop = 0;
					thisclient->wait_validation_graydrop = useitem->quality;
					thisclient->timergraydrop = time (NULL) + 60 * (useitem->affected_Amount * 10);  //Effective time duration
			...
			if (useitem->affected_Stat == 138)		//exp nulifier medal
			{
				thisclient->timer_no_exp = time (NULL) + 60 * (UseList.Index[useitem->id]->affected_Amount * 10);     //Effective Time Duration
				BEGINPACKET (pak, 0x702);
				ADDSTRING (pak, "Null Exp Medal activated.");
				ADDBYTE (pak, 0);
				thisclient->client->SendPacket (&pak);
				flag = true;
			}
		...
		break;
		/// type 322 doesn't come through here. It just gets disassembled
	...
	return true;
	///PY New code ends here. Leaving the rest of it to scavenge code from if something doesn't work correctly
...
bool CWorldServer::pakCraft( CPlayer* thisclient, CPacket* P )
{
	...
	item.lifespan = 99; //Its new so 99% as this is the maximum allowed
	item.refine = 0;
    ...
    switch(item.itemtype)
    {
        case 10:
        {
            ...
            craftID		= UseList.Index[item.itemnum]->craft_skill;
            craft_level	= UseList.Index[item.itemnum]->craft_level;
        }
        break;
        case 11:
        {
            ...
            craftID		= JemList.Index[item.itemnum]->craft_skill;
        ...
        case 14:
        {
            ...
            craftID		= PatList.Index[item.itemnum]->craft_skill;
        ...
        default:
        {
            ...
            {
                failure		= EquipList[item.itemtype].itemList.at (item.itemnum)->craft_difficult;
                craftID		= EquipList[item.itemtype].itemList.at (item.itemnum)->craft_skill;
                craft_level	= EquipList[item.itemtype].itemList.at (item.itemnum)->craft_level;
            }
            else
            {
    ...
    if(!painting)
    {
        ...
        if((GServer->RandNumber(0,100)<=failure)||test_fail)
        {
            ...
            if(item.itemtype<10)
            {
                materialnumber = EquipList[item.itemtype].itemList.at (item.itemnum)->craft_recipe;
            }
            else
            {
                switch(item.itemtype)
                {
                    //UseList by core
                    case 10:materialnumber = UseList.Index[item.itemnum]->craft_recipe;break;
                    case 11:materialnumber = JemList.Index[item.itemnum]->craft_recipe;break;
                    case 14:materialnumber = PatList.Index[item.itemnum]->craft_recipe;break;
                }
            }
...
bool CWorldServer::pakStorage( CPlayer* thisclient, CPacket* P)
{
    ...
    switch(action)
    {
        ...
        case 0x20:
        {
            if(thisclient->Clan == NULL || thisclient->Clan->clanid == 0)
            {
                SendPM(thisclient, "You are not in a clan.");
                return true;
            }
            if(thisclient->Clan->clanrank < 5)
            {
                SendPM(thisclient, "Only Clan Master and Deputy can access Clan Storage.");
                return true;
            }

            MYSQL_RES *result = DB->QStore("SELECT itemnum, itemtype, refine, durability, lifespan, slotnum, count, stats, socketed, appraised, gem FROM clan_storage WHERE clanid=%u", thisclient->Clan->clanid);
            int nitems = result ? mysql_num_rows(result) : 0;
            ...
            if(result != NULL)
            {
                ...
                {
                    CItem item;
                    item.itemnum = atoi(row[0]);
                    item.itemtype = atoi(row[1]);
                    item.refine = atoi(row[2]);
                    item.durability = atoi(row[3]);
                    item.lifespan = atoi(row[4]);
                    int slot = atoi(row[5]);
                    item.count = atoi(row[6]);
                    item.stats = atoi(row[7]);
                    item.socketed = (atoi(row[8]) == 1);
                    item.appraised = (atoi(row[9]) == 1);
                    item.gem = atoi(row[10]);
            ...
            ADDQWORD( pak, thisclient->Clan->money );
            thisclient->client->SendPacket( &pak );
            Log(MSG_INFO, "Player %s opened clan storage for clan %u", thisclient->CharInfo->charname, thisclient->Clan->clanid);
        }
        break;
        default:
    ...
    switch(action)
    {
        case 0x00: //Deposit
        {
            ...
            switch (newitem.itemtype) {
                ...
                {
                    // EQ
                    CEquip* neweq = EquipList[newitem.itemtype].itemList.at (newitem.itemnum);
                    if (neweq == NULL) {
                        storageprice = -1;
                    } else {
        ...
        break;
        case 0x10: // Clan Storage Deposit
        {
            if(thisclient->Clan == NULL || thisclient->Clan->clanid == 0)
                return false;
            ...
            MYSQL_RES* result = DB->QStore("SELECT slotnum, itemnum, itemtype, count FROM clan_storage WHERE clanid=%u ORDER BY slotnum ASC", thisclient->Clan->clanid);
            int target_slot = -1;
            bool slots_used[160] = {false};
            if(result != NULL)
            {
                MYSQL_ROW row;
                while((row = mysql_fetch_row(result)))
                {
                    int s = atoi(row[0]);
                    if(s >= 0 && s < 160) slots_used[s] = true;
                    if(atoi(row[1]) == (int)newitem.itemnum && atoi(row[2]) == (int)newitem.itemtype && newitem.itemtype > 9 && newitem.itemtype < 14)
                    {
                        target_slot = s;
                        break;
                    }
            ...
            if(target_slot == -1)
            {
                SendPM(thisclient, "Clan storage is full.");
                return true;
            }
            ...
            DB->QExecute("INSERT INTO clan_storage (clanid, itemnum, itemtype, refine, durability, lifespan, slotnum, count, stats, socketed, appraised, gem, sp_value) "
                         "VALUES (%u, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i) "
                         "ON DUPLICATE KEY UPDATE count = count + %i",
                         thisclient->Clan->clanid, newitem.itemnum, newitem.itemtype, newitem.refine, newitem.durability, newitem.lifespan,
                         target_slot, newitem.count, newitem.stats, (newitem.socketed?1:0), (newitem.appraised?1:0), newitem.gem, 0,
                         newitem.count);

            Log(MSG_INFO, "Player %s deposited item %i into clan storage slot %i", thisclient->CharInfo->charname, newitem.itemnum, target_slot);

            CPacket refreshPak;
            refreshPak.Command = 0x7ad;
            refreshPak.Size = 8;
            refreshPak.Buffer[0] = 0x10;
            pakStorage(thisclient, &refreshPak);
        }
        break;
        case 0x11: // Clan Storage Withdraw
        {
            if(thisclient->Clan == NULL || thisclient->Clan->clanid == 0)
                return false;
            if(thisclient->Clan->clanrank < 5)
            {
                SendPM(thisclient, "Only Clan Master and Deputy can withdraw from Clan Storage.");
                return true;
            }

            BYTE storageslot = GETBYTE((*P),1);
            if(storageslot >= 160) return false;

            MYSQL_RES* result = DB->QStore("SELECT itemnum, itemtype, refine, durability, lifespan, count, stats, socketed, appraised, gem FROM clan_storage WHERE clanid=%u AND slotnum=%i", thisclient->Clan->clanid, storageslot);
            ...
            item.durability = atoi(row[3]);
            item.lifespan = atoi(row[4]);
            item.count = atoi(row[5]);
            item.stats = atoi(row[6]);
            item.socketed = (atoi(row[7]) == 1);
            item.appraised = (atoi(row[8]) == 1);
            item.gem = atoi(row[9]);
            DB->QFree();

            int newslot = thisclient->GetNewItemSlot(item);
            if(newslot == 0xffff)
            {
                SendPM(thisclient, "Inventory is full.");
                return true;
            }

            DB->QExecute("DELETE FROM clan_storage WHERE clanid=%u AND slotnum=%i", thisclient->Clan->clanid, storageslot);

            thisclient->items[newslot] = item;
            thisclient->UpdateInventory(newslot);
            thisclient->SaveSlot41(newslot);

            Log(MSG_INFO, "Player %s withdrew item %i from clan storage slot %i", thisclient->CharInfo->charname, item.itemnum, storageslot);
...
bool CWorldServer::pakStoreZuly( CPlayer* thisclient, CPacket* P)
{
    ...
    switch(action)
    {
        ...
        case 0x20://Clan Bank Deposit
        {
            ...
            DB->QExecute("UPDATE list_clan SET money=%llu WHERE id=%u", (unsigned long long)thisclient->Clan->money, thisclient->Clan->clanid);
            Log(MSG_INFO, "Player %s deposited %llu Zulies into Clan %u (New Balance: %llu)", thisclient->CharInfo->charname, zuly, thisclient->Clan->clanid, (unsigned long long)thisclient->Clan->money);
        }
        break;
        case 0x21://Clan Bank Withdraw
        {
            if(thisclient->Clan == NULL || thisclient->Clan->clanid == 0)
                return true;
            if(thisclient->Clan->clanrank < 5)
            {
                SendPM(thisclient, "Only Clan Master and Deputy can withdraw Clan funds.");
                return true;
            }
            if(zuly > thisclient->Clan->money)
                return true;
            thisclient->CharInfo->Zulies += zuly;
            thisclient->Clan->money -= zuly;
            ...
            DB->QExecute("UPDATE list_clan SET money=%llu WHERE id=%u", (unsigned long long)thisclient->Clan->money, thisclient->Clan->clanid);
            Log(MSG_INFO, "Player %s withdrew %llu Zulies from Clan %u (New Balance: %llu)", thisclient->CharInfo->charname, zuly, thisclient->Clan->clanid, (unsigned long long)thisclient->Clan->money);
        }
        break;
        default:
            Log( MSG_INFO, "Storage unknown action: %i ", action);
...
bool CWorldServer::pakModifiedItem( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        ...
        case 0x05://Refine
        {
			BYTE itemslot = GETBYTE ((*P), 3);
			BYTE mat1slot = GETBYTE ((*P), 4);
			BYTE mat2slot = GETBYTE ((*P), 5);
			BYTE mat3slot = GETBYTE ((*P), 6);
			BYTE mat4slot = GETBYTE ((*P), 7);
			WORD mat1count = GETWORD ((*P), 8);
			WORD mat2count = GETWORD ((*P), 10);
			WORD mat3count = GETWORD ((*P), 12);
			WORD mat4count = GETWORD ((*P), 14);
			return doRefineItem (thisclient, itemslot, mat1slot, mat2slot, mat3slot, mat4slot, mat1count, mat2count, mat3count, mat4count);
                ...
                UINT gradeIndex = EquipList[thisclient->items[item].itemtype].itemList.at (thisclient->items[item].itemnum)->refineitem;
                ...
                */
                ...
                else
                {
                    ...
                    {
                        quality=EquipList[thisclient->items[item].itemtype].itemList.at (thisclient->items[item].itemnum)->quality;
                    }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            grade = EquipList[thisclient->items[item].itemtype].itemList.at (thisclient->items[item].itemnum)->itemgrade;
                        }
        ...
        case 0x06: // Drill
        {
            ...
            Log(MSG_INFO,"Drill succes? %i/%i",luck_time,UseList.Index[thisclient->items[material].itemnum]->affected_Amount);
            if(luck_time>UseList.Index[thisclient->items[material].itemnum]->affected_Amount)
            {
                //fail
        ...
        case 0x08:
        {
            ...
            if(RandNumber(0,100)>UseList.Index[thisclient->items[material].itemnum]->affected_Amount)
            {
                failed=true;
            }
        ...
        case 0x09:
        {
            ...
            //Ok let's try to see how it's going.
            CItem* newgem = new (nothrow) CItem;
...
bool CWorldServer::pakRepairItem( CPlayer* thisclient, CPacket* P,int packet_type)
{
    ...
    switch( packet_type)
    {
        case 1:
        {
			...
			if (thisclient->items[slot].lifespan > 99)
			{
				SendPM (thisclient, "You cannot repair this item. It is already at the maximum value of 99");
				return true;
			}
            ...
            // Calculate repair cost based on item price and lost condition
            UINT bprice = 0;
            if (thisclient->items[slot].itemtype > 0 && thisclient->items[slot].itemtype < 10)
            {
                if (thisclient->items[slot].itemnum < EquipList[thisclient->items[slot].itemtype].itemList.size() &&
                    EquipList[thisclient->items[slot].itemtype].itemList.at(thisclient->items[slot].itemnum) != NULL)
                {
                    bprice = EquipList[thisclient->items[slot].itemtype].itemList.at(thisclient->items[slot].itemnum)->price;
                }
            }
            else if (thisclient->items[slot].itemtype == 14)
            {
                if (thisclient->items[slot].itemnum < PatList.max && PatList.Index[thisclient->items[slot].itemnum] != NULL)
                {
                    bprice = PatList.Index[thisclient->items[slot].itemnum]->price;
                }
            }

            int lost_lifespan = 99 - thisclient->items[slot].lifespan;
            if (lost_lifespan <= 0)
            {
                SendPM(thisclient, "This item does not need repairs.");
                return true;
            }

            uint64 repair_cost = (uint64)floor((double)bprice * ((double)lost_lifespan / 100.0) * 0.05);
            if (repair_cost < 10)
                repair_cost = 10;

            if ((uint64)thisclient->CharInfo->Zulies < repair_cost)
            {
                SendPM(thisclient, "You need %llu Zulies to repair this item.", (unsigned long long)repair_cost);
                return true;
            }

            thisclient->CharInfo->Zulies -= repair_cost;
        }
        break;
        case 2:
        {
            ...
            if (UseList.Index[thisclient->items[slot_tool].itemnum]->quality <= 50)		//reroll hammer quality is 60 so it doesn't go through here
            {
                //standard quality, so durability goes away...
				if (thisclient->items[slot].lifespan > 99)		//if the item already has 99% then do nothing. just kick it back
				{
					SendPM (thisclient, "You cannot repair this item. It is already at the maximum value of 99");
					return true;
				}
				UINT Raretype = EquipList[thisclient->items[slot].itemtype].itemList.at (thisclient->items[slot].itemnum)->rare_type;

				if (Raretype < 11)
				{ 
					thisclient->items[slot].durability -= 1;
					if(thisclient->items[slot].durability <= 1)
						thisclient->items[slot].durability = 1;
				}
            }
			if (thisclient->items[slot_tool].itemnum == 292)		//PY: hijacked this code to make a reroll hammer
			{
				repair = false;
				if (thisclient->items[slot].stats <= 300)
				{
					int newStat = RandNumber (1, 300);
					thisclient->items[slot].stats = newStat;
					thisclient->items[slot].appraised = true;
					SendPM (thisclient, "Item Stat value has been re-rolled and is now %i", newStat);
				}
				else
				{
					SendPM (thisclient, "Cannot re-roll stats on this item because it has a gem");
					return true;
				}
			}
			// now take the tool away. One of them if its a stack
	...
	if(repair == true)			//repair = false if it's a reroll hammer so don't modify the lifespan
		thisclient->items[slot].lifespan = 99;

    // Send updated Zulies and repaired item info to client
    if(packet_type == 1)
    {
...
bool CWorldServer::pakRepairTool( CPlayer* thisclient, CPacket* P )
{
    ...
    thisclient->items[(GETBYTE((*P), 0x2))].lifespan = 99;
...
bool CWorldServer::GiveChestItems( CPlayer* thisclient,UINT chestSlot, CChest* thischest)
{
    ...
    if (give_all)
    {
        ...
        for(int no_reward=0;no_reward<nb_items;no_reward++)
        {
            ...
            mychest[no_reward].item.lifespan = 99;
   ...
   if(totalprob==0)
   {
    ...
    if(give_all)
    {
        for (int no_reward=0;no_reward<nb_items;no_reward++)
        {
            ...
            mybreak[no_reward].item.lifespan = 99;
...
bool CWorldServer::GiveDefaultDasm( CPlayer* thisclient,UINT src, bool not_found, bool is_failed)
{
   ...
   if(not_found)
   {
        ...
        else
        {
            ...
            {
                grade=EquipList[thisclient->items[src].itemtype].itemList.at (thisclient->items[src].itemnum)->itemgrade;
            }
...
bool CWorldServer::doRefineItem (CPlayer* thisclient, BYTE ItemSlot, BYTE Mat1Slot, BYTE Mat2Slot, BYTE Mat3Slot, BYTE Mat4Slot, WORD Mat1Count, WORD Mat2Count, WORD Mat3Count, WORD Mat4Count) 
{
#define CRAFT_INVALID_ITEM          0x08	// Item is invalid.
#define CRAFT_NOT_ENOUGH_MONEY		0x09
#define	CRAFT_UPGRADE_SUCCESS		0x10	// success
#define	CRAFT_UPGRADE_FAILED		0x11	// fail
#define	CRAFT_UPGRADE_INVALID_MAT	0x12	// Material items was wrong.

	WORD nChangedItemCount = 0;
	BYTE FailCode = 0;
	WORD MatCount = 1;						// Item counts as 1 so we start at 1

	CCraftRecipe* Recipe = new (nothrow) CCraftRecipe;
	CItem* Item = NULL;
	CItem* Mat1 = NULL;
	CItem* Mat2 = NULL;
	CItem* Mat3 = NULL;
	...
	if (!CheckInventorySlot (thisclient, ItemSlot))
	{
		Log (MSG_WARNING, "%s (refine):: wrong slot for item in slot %i", thisclient->CharInfo->charname, ItemSlot);
		FailCode = CRAFT_INVALID_ITEM;
	}

	// First restore the item; see what we're dealing with.
	Item = Item->GetCItem (thisclient->items[ItemSlot].itemtype, thisclient->items[ItemSlot].itemnum);	// most of the time we seem to be missing stuff like refineitem so it fails. Here we import the base STB data from the EquipList to retrieve that stuff
	//now grab the specific changeable stuff
	Item->refine = thisclient->items[ItemSlot].refine;
	Item->durability = thisclient->items[ItemSlot].durability;
	Item->durabLeft = thisclient->items[ItemSlot].durabLeft;
	Item->lifespan = thisclient->items[ItemSlot].lifespan;
	if (Item->lifespan == 100)
		Item->lifespan = 99;		//PY: 99 is the maximum or else reroll hammers don't work

	if (!Item->isValid ()) FailCode = CRAFT_INVALID_ITEM;

	// Check for money
	DWORD Cost = GetItemRefinePrice (Item);
	if (thisclient->CharInfo->Zulies - Cost < 0) FailCode = CRAFT_NOT_ENOUGH_MONEY;
	
	// Determine the refine level:
	WORD currGrade = Item->refine / 16; if (currGrade > 16) currGrade = 16;					// Fix for Refine Level * 16 as we have loads of this.
	WORD nextGrade = currGrade + 1;
	WORD prevGrade1 = 0;
	if (currGrade > 1)
		prevGrade1 = currGrade - 1;
	WORD prevGrade2 = 0;
	if (currGrade > 2)
		prevGrade2 = currGrade - 2;

	WORD gradeIndex = Item->itemgrade;

	if (nextGrade > GServer->Config.max_refine) 
	{
		FailCode = CRAFT_UPGRADE_FAILED;					// We can't refine higher than 9 or 15 (depending on config value);
		char buffer[100];
		sprintf (buffer, "You have attempted to refine beyond the server limit");
		SendPM (thisclient, buffer);
	}

	Recipe = RecipeList.at(Item->refineitem);														// refineitem is actually a link to LIST_PRODUCT.STB. It's a recipe, not an actual item	

	//FailCode = CRAFT_UPGRADE_INVALID_MAT;
	if (!Recipe->isValid ())  FailCode = CRAFT_UPGRADE_FAILED; 

	//Now we check the stuff that is in the slots that are specified in the packet
	if (Mat1Slot != 0 && isValidSlot (thisclient, Mat1Slot))				//Mandatory item Bindrune or Talisman
	{
		Mat1 = Mat1->GetCItem (thisclient->items[Mat1Slot].itemtype, thisclient->items[Mat1Slot].itemnum);
		Mat1->count = thisclient->items[Mat1Slot].count;
		if (Mat1->count < Mat1Count) FailCode = CRAFT_UPGRADE_INVALID_MAT;	//Check if the mat count in the designated inventory slot is enough to cover the requested amount 
		if (Mat1->usetype != 427) FailCode = CRAFT_UPGRADE_INVALID_MAT;		//Check that the mat usetype is correct. Refining Mats should all be type 427
		Log (MSG_WARNING, "%s (refine):: mat1 in slot %i OK", thisclient->CharInfo->charname, Mat1Slot);
		MatCount++;
	}
	else
	{
		FailCode = CRAFT_UPGRADE_INVALID_MAT;								//Invalid mat or slot so FAIL
		Log (MSG_WARNING, "%s (refine):: mat1 Invlaid slot");
	}
	if (Mat2Slot != 0 && isValidSlot (thisclient, Mat2Slot))				//Mandatory item Catalyst
	{
		Mat2 = Mat2->GetCItem (thisclient->items[Mat2Slot].itemtype, thisclient->items[Mat2Slot].itemnum);
		Mat2->count = thisclient->items[Mat2Slot].count;
		if (Mat2->count < Mat2Count) FailCode = CRAFT_UPGRADE_INVALID_MAT;	//Check if the mat count in the designated inventory slot is enough to cover the requested amount
		if (Mat2->usetype != 408) FailCode = CRAFT_UPGRADE_INVALID_MAT;		//Check that the mat usetype is correct. Catalysts should all be type 408
		Log (MSG_WARNING, "%s (refine):: mat2 in slot %i OK", thisclient->CharInfo->charname, Mat2Slot);
		MatCount++;
	}
	else
	{
		FailCode = CRAFT_UPGRADE_INVALID_MAT;								//Invalid Catalyst or slot so FAIL
		Log (MSG_WARNING, "%s (refine):: mat2 Invlaid slot");
	}
	if (Mat3Slot != 0 && isValidSlot (thisclient, Mat3Slot))				//Optional item. no automatic fail. Venurunes go here
	{
		Mat3 = Mat3->GetCItem (thisclient->items[Mat3Slot].itemtype, thisclient->items[Mat3Slot].itemnum);
		Mat3->count = thisclient->items[Mat3Slot].count;
		if (Mat3->count < Mat3Count) FailCode = CRAFT_UPGRADE_FAILED;		//Check if the mat count in the designated inventory slot is enough to cover the requested amount
		if (Mat3->usetype != 410) FailCode = CRAFT_UPGRADE_INVALID_MAT;		//Check that the mat usetype is correct. Refine Enhancement items should all be type 410
		Log (MSG_WARNING, "%s (refine):: mat3 in slot %i OK", thisclient->CharInfo->charname, Mat3Slot);
		MatCount++;
	}
	else
	{
		Log (MSG_WARNING, "%s (refine):: mat3 Invlaid slot. Not a critical fail", thisclient->CharInfo->charname);
	}
	if (Mat4Slot != 0 && isValidSlot (thisclient, Mat4Slot))				//Optional item. no automatic fail. Plutorunes or NeptuRunes go here
	{
		Mat4 = Mat4->GetCItem (thisclient->items[Mat4Slot].itemtype, thisclient->items[Mat4Slot].itemnum);
		Mat4->count = thisclient->items[Mat4Slot].count;
		if (Mat4->count < Mat4Count) FailCode = CRAFT_UPGRADE_FAILED;		//Check if the mat count in the designated inventory slot is enough to cover the requested amount
		if (Mat4->usetype != 409) FailCode = CRAFT_UPGRADE_INVALID_MAT;	//Check that the mat usetype is correct. Refine Protection items should all be type 409
		Log (MSG_WARNING, "%s (refine):: mat4 in slot %i failed", thisclient->CharInfo->charname, Mat4Slot);
		MatCount++;
	}
	else
	{
		Log (MSG_WARNING, "%s (refine):: mat4 Invlaid slot. Not a critical fail", thisclient->CharInfo->charname);
	}
	
	if( FailCode == 0) 
	{
		// Validate the ingredients:
		bool bOk = Recipe->CheckIngredients (Mat1, Mat2, Mat3, Mat4);			// Note that the actual recipe only includes the Bindrune / Talisman. Catalysts are not included so they will not be checked here.

		if (bOk)	// We know what kind of refine item is needed now so we can start working on refining the item
		{
			// We can refine; get the success Chance;
			uint16 Chance = GetItemRefineProb (Item);
			DWORD Cost = GetItemRefinePrice (Item);

			thisclient->CharInfo->Zulies -= Cost;
			// Take the money either way.
			BEGINPACKET (pak, 0x71d);
			ADDQWORD (pak, thisclient->CharInfo->Zulies);
			thisclient->client->SendPacket (&pak);

			//Adjust the success chance by means of catalysts
			//The bonus given by a catalyst is 1/10 of it's quality
			Chance = Chance * 10;							// we seem to be locked into uint variables and we really need a float here so I'm multiplying it all by 10 so it's chances in 1000 rather than 100
			Chance += Mat2->quality * Mat2Count;			// Now we don't need decimal places ^_^. Use the quality of the powder or lisent  and the number used to adjust success chance
			if (Mat3 != NULL)
				if (Mat3->usetype == 410) Chance += (Mat3->refine * 10);						//Venurunes have a refine bonus value (also Arua and Hebarn fates. All type 410 )
			if (Mat4 != NULL)
				if (Mat4->usetype == 409 && Mat4->refine != 0) Chance += (Mat4->refine * 10);	//Some type 409s also have a refine bonus (Nepturune) but not all though (plutorunes) so we have to test for it

			// Let's see if we have success.
			bool bSuccess = (Rose::Support::Random (1, 1000) <= Chance);						//Randomize from 1 to 1000 so we correct for the 10X multiplication above

			if(thisclient->Session->codedebug)
			{ 
				bSuccess = true;			//for testing. use /debugmode to force it true but ONLY for thisclient
			}
			if (bSuccess)		//we succeeded
			{
				// Fix the Item:
				WORD nDuraAdd = Rose::Support::Random (0, WORD (((gradeIndex + 1) *1.5f) + 1.5f));
				thisclient->items[ItemSlot].refine = nextGrade * 16;
				thisclient->items[ItemSlot].lifespan = 99;
				thisclient->items[ItemSlot].durability += nDuraAdd;

				// Cap Dura at 120;
				if (thisclient->items[ItemSlot].durability > 120) thisclient->items[ItemSlot].durability = 120;

				//Log (MSG_INFO, "Refine item %s Success, New Refine: %i, New Dura:%i", gItem->toString ().c_str (), nextGrade, thisclient->items[ItemSlot].durability);

				// Decrement the items because we still pay the cost.
				if (Mat1 != NULL) thisclient->items[Mat1Slot].Decrement (Mat1Count);
				if (Mat2 != NULL) thisclient->items[Mat2Slot].Decrement (Mat2Count);
				if (Mat3 != NULL) thisclient->items[Mat3Slot].Decrement (Mat3Count);
				if (Mat4 != NULL) thisclient->items[Mat4Slot].Decrement (Mat4Count);
				
				FailCode = CRAFT_UPGRADE_SUCCESS;
				nChangedItemCount =  MatCount;	

				// Send back the  item, and all changed materials.
				RESETPACKET (pak, 0x7bc);
				ADDBYTE		(pak, CRAFT_UPGRADE_SUCCESS);
				ADDBYTE		(pak, 1 + MatCount);

				if (MatCount > 1)
				{
					ADDBYTE		(pak, Mat1Slot);
					ADDDWORD	(pak, BuildItemHead ( &thisclient->items[Mat1Slot]));
					ADDDWORD	(pak, BuildItemData ( &thisclient->items[Mat1Slot]));
					ADDDWORD	(pak, 0x00000000);
					ADDWORD		(pak, 0x0000);
					//pak.addItem (thisclient->items[Mat1Slot]);
				}

				if (MatCount > 2)
				{
					ADDBYTE		(pak, Mat2Slot);
					ADDDWORD	(pak, BuildItemHead ( &thisclient->items[Mat2Slot]));
					ADDDWORD	(pak, BuildItemData ( &thisclient->items[Mat2Slot]));
					ADDDWORD	(pak, 0x00000000);
					ADDWORD		(pak, 0x0000);
					//pak.addItem (thisclient->items[Mat2Slot]);
				}

				if (MatCount > 3)
				{
					ADDBYTE		(pak, Mat3Slot);
					ADDDWORD	(pak, BuildItemHead ( &thisclient->items[Mat3Slot]));
					ADDDWORD	(pak, BuildItemData ( &thisclient->items[Mat3Slot]));
					ADDDWORD	(pak, 0x00000000);
					ADDWORD		(pak, 0x0000);
					//pak.addItem (thisclient->items[Mat3Slot]);
				}

				if (MatCount > 4)
				{
					ADDBYTE		(pak, Mat4Slot);
					ADDDWORD	(pak, BuildItemHead ( &thisclient->items[Mat4Slot]));
					ADDDWORD	(pak, BuildItemData ( &thisclient->items[Mat4Slot]));
					ADDDWORD	(pak, 0x00000000);
					ADDWORD		(pak, 0x0000);
					//pak.addItem (thisclient->items[Mat4Slot]);
				}
				ADDBYTE (pak, ItemSlot);
				ADDDWORD (pak, BuildItemHead (&thisclient->items[ItemSlot]));
				ADDDWORD (pak, BuildItemData (&thisclient->items[ItemSlot]));
				ADDDWORD (pak, 0x00000000);
				ADDWORD (pak, 0x0000);
			...
			{
				/// In case of Failure we need to manually update the money (server side) and items (Server + Client Side).
				/// We already took the money so just handle the items here
				// Decrement the items.
				if (Mat1 != NULL) thisclient->items[Mat1Slot].Decrement (Mat1Count);
				if (Mat2 != NULL) thisclient->items[Mat2Slot].Decrement (Mat2Count);
				if (Mat3 != NULL) thisclient->items[Mat3Slot].Decrement (Mat3Count);
				if (Mat4 != NULL) thisclient->items[Mat4Slot].Decrement (Mat4Count);
				//Now we determine if we should downgrade or destroy the item
				if (nextGrade < 6)						// degrade 1 unless protected by Plutorune
				{ 
					if(Mat4 == NULL)					//degrade 1 if there's nothing in the pluto/nepturune slot
					{ 
						thisclient->items[ItemSlot].refine = prevGrade1 * 16;
					}
				}
				if (nextGrade > 5 && nextGrade < 11)	// Destroy item unless protected by Plutorune but still degrades 1
				{
					if (Mat4 == NULL)					//destroy item if there's nothing in the pluto/nepturune slot
					{
						thisclient->items[ItemSlot].Clear ();
					}
					else
					{
						thisclient->items[ItemSlot].refine = prevGrade1 * 16;	// down 1
					}
				}
				if (nextGrade > 10 && nextGrade < 16)	// Destroy item unless protected by Plutorune but still degrades 2
				{
					if (Mat4 == NULL)					//destroy item if there's nothing in the pluto/nepturune slot
					{
						thisclient->items[ItemSlot].Clear ();
					...
					{
						thisclient->items[ItemSlot].refine = prevGrade2 * 16;	// down 2
					}
				}

				RESETPACKET (pak, 0x7bc);
				ADDBYTE		(pak, CRAFT_UPGRADE_FAILED);
				ADDBYTE		(pak, 1 + MatCount);
				...
				/*CPacket pak (0x7bc);
				pak.addBYTE (CRAFT_UPGRADE_FAILED);
				pak.addBYTE (2);

				pak.addBYTE (ItemSlot);
				pak.addItem (thisclient->items[ItemSlot]);

				pak.addBYTE (MatSlot);
				pak.addItem (thisclient->items[MatSlot]);*/
		...
		{
			// We don't have the materials it takes to complete the refine.
			// Send out the failure code;
			BEGINPACKET (pak, 0x7bc);
			ADDBYTE		(pak, CRAFT_UPGRADE_INVALID_MAT);
			ADDBYTE		(pak, 1);		// Item + the changed material.
			ADDBYTE (pak, ItemSlot);
			ADDDWORD (pak, BuildItemHead (&thisclient->items[ItemSlot]));
			ADDDWORD (pak, BuildItemData (&thisclient->items[ItemSlot]));
			ADDDWORD (pak, 0x00000000);
	...
	else 
	{
		...
		// Some other error has occured. Maybe we attempted to refine beyond the config limit
		BEGINPACKET (pak, 0x7bc);
		ADDBYTE		(pak, FailCode);
		ADDBYTE		(pak, 1);		// Item + the changed material.
		ADDBYTE (pak, ItemSlot);
		ADDDWORD (pak, BuildItemHead (&thisclient->items[ItemSlot]));
		ADDDWORD (pak, BuildItemData (&thisclient->items[ItemSlot]));

...
int STLStoreDataFile (const char* filename, std::map<uint32, char*>&Text) {
	entry** index;
	...
	Log (MSG_LOAD, "Loading %s", filename);
	...
	for (UINT j = 0; j < entryCount; j++) {
		...
		Text[mapid] = new_text;
		//Comment[mapid] = new_comment;	/// We really don't need the comments, that's client only.
	}

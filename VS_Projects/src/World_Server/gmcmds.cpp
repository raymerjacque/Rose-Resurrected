...
bool CWorldServer::pakGMCommand( CPlayer* thisclient, CPacket* P )
{
	...
	if (strcmp(command, "bot") == 0)
    {
        ...
        if (subcmd == NULL || strcmp(subcmd, "help") == 0)
        {
            SendPM(thisclient, "=== ROSE Online PlayerBot Commands ===");
            SendPM(thisclient, "/bot spawn <name> [job] [lvl] - Spawn a bot");
            SendPM(thisclient, "/bot remove <name> - Remove a bot");
            SendPM(thisclient, "/bot list - List all active bots");
            SendPM(thisclient, "/bot follow <name|all> - Bot follows you");
        ...
        if (strcmp(subcmd, "spawn") == 0)
        {
            ...
            {
                sprintf(defaultName, "Bot_%u", (unsigned int)(CBotManager::GetInstance()->GetBotCount() + 1));
                name = defaultName;
            }

            int job = 111; // Default Soldier
            if (jobStr)
            {
                std::string js = jobStr;
                for (size_t k = 0; k < js.length(); k++) js[k] = tolower(js[k]);
                if (js == "visitor" || js == "0") job = 0;
                else if (js == "soldier" || js == "111") job = 111;
                else if (js == "knight" || js == "121") job = 121;
                else if (js == "champion" || js == "122") job = 122;
                else if (js == "muse" || js == "211") job = 211;
                else if (js == "mage" || js == "221") job = 221;
                else if (js == "cleric" || js == "222") job = 222;
                else if (js == "hawker" || js == "311") job = 311;
                else if (js == "raider" || js == "321") job = 321;
                else if (js == "scout" || js == "322") job = 322;
                else if (js == "dealer" || js == "411") job = 411;
                else if (js == "bourg" || js == "bourgeois" || js == "421") job = 421;
                else if (js == "artisan" || js == "422") job = 422;
                else
                {
                    int parsed = atoi(jobStr);
                    if (parsed > 0) job = parsed;
                }
            }

            int level = thisclient->Stats->Level;
            if (lvlStr)
            {
                int parsedLvl = atoi(lvlStr);
                if (parsedLvl >= 1 && parsedLvl <= 200) level = parsedLvl;
            }

            fPoint spawnPos = thisclient->Position->current;
            spawnPos.x += 1.5f;
            spawnPos.y += 1.5f;

            CPlayer* bot = CBotManager::GetInstance()->SpawnBot(name, job, level, thisclient->Position->Map, spawnPos);
            if (bot)
            {
                SendPM(thisclient, "[PlayerBot] Bot '%s' spawned (Job: %d, Lvl: %d) at (%.1f, %.1f)!",
                       bot->CharInfo->charname, bot->CharInfo->Job, bot->Stats->Level, spawnPos.x, spawnPos.y);
            }
            else
            {
                SendPM(thisclient, "[PlayerBot] Failed to spawn bot '%s'. Name may be in use.", name);
            }
            return true;
        }
        ...
        if (strcmp(subcmd, "remove") == 0)
        {
            ...
            if (!name)
            {
                SendPM(thisclient, "[PlayerBot] Usage: /bot remove <name>");
                return true;
            }
            bool removed = CBotManager::GetInstance()->RemoveBot(name);
            if (removed)
            {
                SendPM(thisclient, "[PlayerBot] Bot '%s' removed successfully.", name);
            }
            else
            {
                SendPM(thisclient, "[PlayerBot] Bot '%s' not found.", name);
            }
            return true;
        }
        ...
        if (strcmp(subcmd, "list") == 0)
        {
            ...
            SendPM(thisclient, "[PlayerBot] Active bots: %zu", bots.size());
            for (size_t i = 0; i < bots.size(); i++)
            {
                CPlayerBot* b = bots[i];
                if (b && b->GetPlayer())
                {
                    CPlayer* bp = b->GetPlayer();
                    SendPM(thisclient, "[Bot #%zu] %s | Lvl %d | Job %d | Map %d | HP: %I64i/%d | State: %s",
                           i + 1, bp->CharInfo->charname, bp->Stats->Level, bp->CharInfo->Job,
                           bp->Position->Map, bp->Stats->HP, bp->Stats->MaxHP, b->GetStateString());
                }
            }
            return true;
        }

        if (strcmp(subcmd, "follow") == 0)
        {
            char* name = strtok(NULL, " ");
            if (!name)
            {
                SendPM(thisclient, "[PlayerBot] Usage: /bot follow <name|all>");
                return true;
            }
            if (strcmp(name, "all") == 0)
            {
                ...
                for (size_t i = 0; i < bots.size(); i++)
                {
                    if (bots[i] && bots[i]->GetPlayer() && bots[i]->GetPlayer()->Position->Map == thisclient->Position->Map)
                    {
                        bots[i]->SetFollowTarget(thisclient);
                        bots[i]->SetState(BOT_STATE_FOLLOW);
                        count++;
                    }
                }
                SendPM(thisclient, "[PlayerBot] %d bots are now following you.", count);
            }
            else
            {
                CPlayerBot* b = CBotManager::GetInstance()->GetBot(name);
                if (b)
                {
                    b->SetFollowTarget(thisclient);
                    b->SetState(BOT_STATE_FOLLOW);
                    SendPM(thisclient, "[PlayerBot] Bot '%s' is now following you.", name);
                }
                else
                {
                    SendPM(thisclient, "[PlayerBot] Bot '%s' not found.", name);
        ...
        if (strcmp(subcmd, "roam") == 0 || strcmp(subcmd, "auto") == 0)
        {
            char* name = strtok(NULL, " ");
            if (!name || strcmp(name, "all") == 0)
            {
                ...
                for (size_t i = 0; i < bots.size(); i++)
                {
                    if (bots[i] && bots[i]->GetPlayer())
                    {
                        bots[i]->SetFollowTarget(NULL);
                        bots[i]->SetRoamCenter(bots[i]->GetPlayer()->Position->current);
                        bots[i]->SetAutoRoam(true);
                        bots[i]->SetState(BOT_STATE_IDLE);
                    }
                }
                SendPM(thisclient, "[PlayerBot] All bots set to autonomous roaming and grinding.");
            }
            else
            {
                CPlayerBot* b = CBotManager::GetInstance()->GetBot(name);
                if (b && b->GetPlayer())
                {
                    b->SetFollowTarget(NULL);
                    b->SetRoamCenter(b->GetPlayer()->Position->current);
                    b->SetAutoRoam(true);
                    b->SetState(BOT_STATE_IDLE);
                    SendPM(thisclient, "[PlayerBot] Bot '%s' set to autonomous roaming.", name);
                }
                else
                {
                    SendPM(thisclient, "[PlayerBot] Bot '%s' not found.", name);
        ...
        if (strcmp(subcmd, "stay") == 0 || strcmp(subcmd, "idle") == 0)
        {
            char* name = strtok(NULL, " ");
            if (!name || strcmp(name, "all") == 0)
            {
                ...
                for (size_t i = 0; i < bots.size(); i++)
                {
                    ...
                    {
                        bots[i]->SetFollowTarget(NULL);
                        bots[i]->SetAutoRoam(false);
                        bots[i]->StopMoving();
                        bots[i]->SetState(BOT_STATE_IDLE);
                    }
                }
                SendPM(thisclient, "[PlayerBot] All bots set to stay idle.");
            }
            else
            {
                CPlayerBot* b = CBotManager::GetInstance()->GetBot(name);
                if (b)
                {
                    ...
                    b->StopMoving();
                    b->SetState(BOT_STATE_IDLE);
                    SendPM(thisclient, "[PlayerBot] Bot '%s' is now staying idle.", name);
                }
                else
                {
                    SendPM(thisclient, "[PlayerBot] Bot '%s' not found.", name);
        ...
        if (strcmp(subcmd, "attack") == 0)
        {
            if (thisclient->Battle->target == 0)
            {
                SendPM(thisclient, "[PlayerBot] You must target a monster first!");
                return true;
            }
            CMap* map = GServer->MapList.Index[thisclient->Position->Map];
            if (!map) return true;
            CMonster* target = map->GetMonsterInMap(thisclient->Battle->target);
            if (!target || target->IsDead())
            {
                SendPM(thisclient, "[PlayerBot] Target monster is invalid or dead.");
                return true;
            }
            ...
            if (!name || strcmp(name, "all") == 0)
            {
                ...
                for (size_t i = 0; i < bots.size(); i++)
                {
                    if (bots[i] && bots[i]->GetPlayer() && bots[i]->GetPlayer()->Position->Map == thisclient->Position->Map)
                    {
                        bots[i]->AttackTarget(target);
                        bots[i]->SetState(BOT_STATE_COMBAT);
                        count++;
                    }
                }
                SendPM(thisclient, "[PlayerBot] %d bots commanded to attack target!", count);
            }
            else
            {
                CPlayerBot* b = CBotManager::GetInstance()->GetBot(name);
                if (b)
                {
                    ...
                    b->SetState(BOT_STATE_COMBAT);
                    SendPM(thisclient, "[PlayerBot] Bot '%s' attacking target!", name);
                }
                else
                {
                    SendPM(thisclient, "[PlayerBot] Bot '%s' not found.", name);
        ...
        if (strcmp(subcmd, "clear") == 0)
        {
            ...
            SendPM(thisclient, "[PlayerBot] All bots removed.");
            return true;
        }

        SendPM(thisclient, "[PlayerBot] Unknown subcommand '%s'. Type '/bot help' for commands.", subcmd);
        return true;
    }
    ...
    if (strcmp(command, "cart")==0)     // get all castlegear parts
    {
        ...
        BEGINPACKET( pak, 0);
        {
            ...
            while( row = mysql_fetch_row(result) )
            {
                ...
                thisclient->items[good_slot].lifespan = 99;
   ...
   // *** RELOAD DATA FILES ******
	...
	if (strcmp (command, "pexp") == 0)
	{
		if ( thisclient->Session->accesslevel < 300 )
			return true;
		if (thisclient->Party->party == NULL)
		{
			SendPM (thisclient, "You are not currently in a party");
			return true;
		}
		CParty* party = thisclient->Party->party;
		SendPM (thisclient, "Your current party level is %i", thisclient->Party->party->PartyLevel);
		SendPM (thisclient, "Your current party exp is %i", thisclient->Party->party->Exp);
	}
	...
	if (strcmp (command, "pexp") == 0) /* Remove all Skill from a Player - modified by rl2171 */
	{
...
//		ADDSTRING( pak, "DELETESPAWN" );
        ...
        for(UINT i=0;i<map->MonsterList.size();i++)
        {
            ...
            MapList.Index[thisspawn->map]->DeleteMonster( thismon, true, i );
        }
    ...
    if (strcmp(command, "drop")==0)
    {
		...
		thisdrop->item = new (nothrow) CItem;
		Clearitem(thisdrop->item);
		thisdrop->item = thisdrop->item->GetCItem (itemtype, itemid);	// get data for the base version of the item
	...
	if (strcmp (command, "setgroup") == 0)		//Assign a player to a group by name
	{
		if (900 > thisclient->Session->accesslevel)
			return true;
		WORD groupid = 0;
		if ((tmp = strtok (NULL, " ")) == NULL) return true; char* name = tmp;
		if ((tmp = strtok (NULL, " ")) == NULL) return true; groupid = atoi (tmp);
		SendPM (thisclient, "Player %s assigned to group %i", name, groupid);
		return pakSetGroup(name, groupid);
	}
	if (strcmp (command, "groupgo") == 0)		//using the GO list, teleport a group to a location
	{
		if (900 > thisclient->Session->accesslevel)
			return true;
		WORD groupid = 0;
		WORD map = 0;
		if ((tmp = strtok (NULL, " ")) == NULL) return true; map = atoi(tmp);
		if ((tmp = strtok (NULL, " ")) == NULL) return true; groupid = atoi (tmp);
		return pakTeleGroup (thisclient, map, groupid);
	}
	if (strcmp (command, "grouptome") == 0)		//Teleports all members of a designated group to your location
	{
		if (900 > thisclient->Session->accesslevel)
			return true;
		WORD groupid = 0;
		if ((tmp = strtok (NULL, " ")) == NULL) return true; groupid = atoi (tmp);
		return pakTeleGroupToMe (thisclient, groupid);
	}
	if (strcmp (command, "groupbuff") == 0)		//buffs all members of a designated group 
	{
		if (900 > thisclient->Session->accesslevel)
			return true;
		WORD groupid = 0;
		WORD buffPower = 0;
		if ((tmp = strtok (NULL, " ")) == NULL) return true; groupid = atoi (tmp);
		if ((tmp = strtok (NULL, " ")) == NULL) return true; buffPower = atoi (tmp);
		return pakBuffGroup (thisclient, groupid, buffPower);
	}
	if (strcmp (command, "statusgroup") == 0)        //Reports status of members of a designated group to your location
	{
		if (900 > thisclient->Session->accesslevel)
			return true;
		WORD groupid = 0;
		if ((tmp = strtok (NULL, " ")) == NULL) return true; groupid = atoi (tmp);
		return pakStatusGroup (thisclient, groupid);
	}
	...
	if (strcmp (command, "go") == 0) // AtCommandGo
	{
		...
		if ((tmp = strtok (NULL, " ")) == NULL)
			...
			loc = atoi (tmp);
		if (loc != 0)		//we have a destination
		{
			fPoint Coord;
			for (UINT i = 0; i < GoList.size (); i++)
			{
				GoCommand *thisCommand = GoList.at (i);
				if (thisCommand == NULL) return true;
				if (thisCommand->Go_id == loc)
				{
					if (thisCommand->Isactive == 1 && thisclient->Session->accesslevel >= thisCommand->Access_lvl)
					{
						if (thisclient->Stats->Level >= thisCommand->Lvl_min)
						{
							SendPM (thisclient, "teleport to map: %i", thisCommand->Mapid);
							MapList.Index[thisCommand->Mapid]->TeleportPlayer (thisclient, thisCommand->Coord, false);
							return true;
						}
						else
						{
							SendPM (thisclient, "teleport to map: %i denied. Level too low", thisCommand->Mapid);
							return true;
						}
					}
					else
					{
						SendPM (thisclient, "Sorry! You do not have necessary permission to go to that map");
						return true;
					}
				}
			}
		...
		{						// we don't have a destination so send the player a list of possible locations
			SendPM (thisclient, "Please input a number after the go command, below is a list of places and their appropriate number");
			for (UINT i = 0; i < GoList.size (); i++)
			{
				GoCommand *thisCommand = GoList.at (i);
				if (thisCommand == NULL) return true;
				SendPM (thisclient, "%i = %s",thisCommand->Go_id, thisCommand->MapName.c_str() );
			}
		}
		return true;
	}
...
//        return true;
 //   }
	...
	if (strcmp (command, "hurtme") == 0)
	{
		if (Config.Command_HurtHim > thisclient->Session->accesslevel)
			return true;
		//if ((tmp = strtok (NULL, " ")) == NULL) return true; char* name = tmp;
		char* name = thisclient->CharInfo->charname;
		Log (MSG_GMACTION, " %s : /hurthim %s", thisclient->CharInfo->charname, name);
		return pakGMHurtHim (thisclient, name);
	}
    ...
    if (strcmp(command, "item")==0)//Modified by Hiei (added refine/socket/stats)
    {
        if(Config.Command_Item > thisclient->Session->accesslevel)
	       return true;
        UINT itemrefine, itemstats, itemdura, itemsocket;
        if ((tmp = strtok(NULL, " "))==NULL) return true; UINT itemid = atoi(tmp);
        if ((tmp = strtok(NULL, " "))==NULL) return true; UINT itemtype = atoi(tmp);
        if ((tmp = strtok(NULL, " "))==NULL) return true; UINT itemamount = atoi(tmp);
        ...
        if ((tmp = strtok(NULL, " "))==NULL)
            itemdura = 0;					// gets set to STB value later so if it's unspecified in the command we make it 0 here
        else
            itemdura = atoi(tmp);
        if ((tmp = strtok(NULL, " "))==NULL)
            itemsocket = 0;
        ...
        if ((tmp = strtok(NULL, " "))==NULL)
            ...
            itemstats = atoi(tmp);
        // If you want GM to socket anything but Armor, Jewelery or Weapons set it in the config table
        if (Config.more_sockets !=1)
        {
        ...
        return pakGMItem( thisclient , itemid , itemtype , itemamount , itemrefine , itemdura, itemstats , itemsocket );
    }
...
//        if(thisclient->Session->accesslevel < 300)
	...
	if(strcmp(command, "settestquests")==0)
    {
        ...
        thisclient->quest.quests[4].Items[0].lifespan		= 99;
        ...
        thisclient->quest.quests[4].Items[4].lifespan		= 99;
	...
	if (strcmp (command, "npcdir") == 0) {
		...
		if (!npctype == 0) {
			CNPC* thisNPC = MapList.Index[map]->GetNPCInMapByType (npctype);
			if (thisNPC == NULL) {
				SendPM (thisclient, "Could not find an instance of NPC %i in this map", npctype);
				return true;
			}
			SendPM (thisclient, "NPC %s rotated to angle %f", thisNPC->name.c_str (), dir);
			return pakSetNPCDir (thisclient, thisNPC, dir);
		}
		else {
			SendPM (thisclient, "You forgot to add npctype to your command");
		}
		return true;
	}
	if (strcmp (command, "movenpc") == 0) {				//moves the selected NPC to my exact position
		if (Config.Command_Npc > thisclient->Session->accesslevel)
			return true;
		UINT npctype = 0;
		if ((tmp = strtok (NULL, " ")) != NULL)
		...
		if (!npctype == 0) {
			...
			pakMoveNPC (thisclient, thisNPC);
			SendPM (thisclient, "NPC %s has been moved", thisNPC->name.c_str ());
		}
		else {
			SendPM (thisclient, "You forgot to add npctype to your command");
		}
	...
	/// WARNING!! This is a PERMANENT Change in your database
	if (strcmp (command, "savenpc") == 0) {					//saves position and facing of specified NPC to the database. 
		if (Config.Command_Npc > thisclient->Session->accesslevel)
			return true;
		UINT npctype = 0;
		if ((tmp = strtok (NULL, " ")) != NULL)
		...
		if (!npctype == 0) {
			...
			pakSaveNPCPos (thisNPC);
			SendPM (thisclient, "NPC %s position data saved to the database", thisNPC->name.c_str ());
		}
		else {
			SendPM (thisclient, "You forgot to add npctype to your command");
		}
	...
	/// WARNING!! SO IS THIS. it will ADD a new npc to your database
	if (strcmp (command, "addnpc") == 0) {					//saves position and facing of specified NPC to the database. 
		if (Config.Command_Npc > thisclient->Session->accesslevel)
			return true;
		UINT npctype = 0;
		if ((tmp = strtok (NULL, " ")) != NULL)
		...
		if (!npctype == 0) {
			...
			pakAddNPC (thisclient, thisNPC);
			SendPM (thisclient, "NPC %s position data added to the database", thisNPC->name.c_str ());
		}
		else {
			SendPM (thisclient, "You forgot to add npctype to your command");
		}
...
//         }
         ...
         else if (strcmp (tmp, "npcstb") == 0)
         {
             STBStoreDataNPC ("3DData\\STB\\LIST_NPC.STB", &STB_NPC);           //reload the STB
             LoadNPCData ();
             SendPM (thisclient, "All NPC Data Reloaded.");
         }
    ...
    if (strcmp(command, "set") == 0)
    {
        ...
        if(EquipList[2].itemList.at (hatid) != NULL)
        {
    		...
    		thisclient->items[2].lifespan = 99;
    	...
    	if(EquipList[3].itemList.at (id) != NULL)
    	{
    		...
    		thisclient->items[3].lifespan = 99;
        ...
        if(EquipList[4].itemList.at (id) != NULL)
        {
    		...
    		thisclient->items[5].lifespan = 99;
        ...
        if(EquipList[5].itemList.at (id) != NULL)
        {
    		...
    		thisclient->items[6].lifespan = 99;
...
{
    CItem item;
	item = *item.GetCItem (itemtype, itemid);	// get data for the base version of the item
	if (itemdura != 0) //getCItem sets the default value to whatever is in the STB
	{ 
		item.durability = itemdura; 
	}	
	// the rest of this stuff is not covered by default STB values
	item.count          = itemamount;
    item.itemnum        = itemid;
    item.itemtype       = itemtype;
    item.lifespan       = 99;  // changed from itemls to 99, no need for lifespan on a item that is spawned - rl2171
...
bool CWorldServer::pakGMItemQuest( CPlayer* thisclient, UINT itemid, UINT itemtype, UINT itemamount, UINT itemrefine, UINT itemls, UINT itemstats, UINT itemsocket, char buffer2[200] )
{
    ...
    item->itemnum			= itemid;
    item->itemtype			= itemtype;
    item->lifespan			= 99;  // changed from items to 99, no need for lifespan on a item that is spawned - rl2171
    item->refine			= itemrefine;
    item->stats				= itemstats;
    item->socketed			= itemsocket;
    item->appraised			= 1;
...
//    }
//
//     if(thisclient->Stats->Level < GServer->Config.MaxLevel) //Level 260 can be changed to any level you want in list_config
//     {
...
bool CWorldServer::pakGMLevel( CPlayer* thisclient, int level, char* name )
{
    ...
    SendPM (thisclient, "you have gained %i levels", level);
...
//Set Group
bool CWorldServer::pakSetGroup (char* name, WORD groupid)
{
	CPlayer* otherclient = GetClientByCharName (name);
	if (otherclient == NULL)
		return true;
	otherclient->group = groupid;
	GServer->DB->QExecute ("UPDATE characters SET groupid = %i WHERE char_name = '%s'", groupid, name);
	return true;
}
// teleports all members of a group (including yourself if you are a member) to a position on the GO list
// Decided to do it in 2 stages. Move yourself first with a GO then bring the group to you
bool CWorldServer::pakTeleGroup (CPlayer* thisclient, WORD go, WORD groupid)
{
	fPoint Coord;
	for (UINT i = 0; i < GoList.size (); i++)
	{
		GoCommand *thisCommand = GoList.at (i);
		if (thisCommand == NULL) return true;
		if (thisCommand->Go_id == go)
		{
			if (thisCommand->Isactive == 1 && thisclient->Session->accesslevel >= thisCommand->Access_lvl)
			{
				SendPM (thisclient, "teleporting group %i to map: %i", groupid, thisCommand->Mapid);
				MapList.Index[thisCommand->Mapid]->TeleportPlayer (thisclient, thisCommand->Coord, false);
				pakTeleGroupToMe ( thisclient, groupid);
				return true;
			}
		}
	}
	SendPM (thisclient, "teleporting group failed!! GO id %i not found in the GO list", go);
	return true;
}
//teleports all members of a group to my location. Ignores me if I am also a member of the group
bool CWorldServer::pakTeleGroupToMe (CPlayer* thisclient, int groupid)
{
	int count = 1;
	CMap* map = MapList.Index[thisclient->Position->Map];
	while (count <= (ClientList.size () - 1))
	{
		...
		if (otherclient != thisclient && otherclient->group == groupid)
		{
			map->TeleportPlayer (otherclient, thisclient->Position->current, false);
		}
		count++;
...
bool CWorldServer::pakBuffGroup (CPlayer* thisClient, WORD groupid, WORD buffPower)  //This buffs your group wherever they are, in ANY map
{
	int count = 1;
	CMap* map = MapList.Index[thisClient->Position->Map];
	while (count <= (ClientList.size () - 1))
	{
		...
		if (otherClient != thisClient && otherClient->group == groupid)
		{
			pakGMGiveBuff (thisClient, otherClient, 3906, buffPower); // Attack   (300s) (18)
			pakGMGiveBuff (thisClient, otherClient, 3905, buffPower); // Defense  (300s) (19)
			pakGMGiveBuff (thisClient, otherClient, 3908, buffPower); // Accuracy (420s) (20)
			pakGMGiveBuff (thisClient, otherClient, 3907, buffPower); // MResist  (420s) (21)
			pakGMGiveBuff (thisClient, otherClient, 3909, buffPower); // Dodge    (420s) (22)
			pakGMGiveBuff (thisClient, otherClient, 3904, buffPower); // Dash     (300s) (23)
			pakGMGiveBuff (thisClient, otherClient, 3910, buffPower); // Haste    (420s) (24)
			pakGMGiveBuff (thisClient, otherClient, 3911, buffPower); // Critical (420s) (26)
			pakGMGiveBuff (thisClient, otherClient, 3900, buffPower); // Max HP   (420s) (38)
			pakGMGiveBuff (thisClient, otherClient, 3901, buffPower); // Max MP   (420s) (39)
			pakGMGiveBuff (thisClient, otherClient, 3912, buffPower); //additional damage (420s) (36)
		}
		count++;
	}
	return true;
...
//displays status of groupID members
bool CWorldServer::pakStatusGroup (CPlayer* thisclient, int groupid)
{
	int count = 1;
	int statcheckbool = 0;
	CMap* map = MapList.Index[thisclient->Position->Map];
	while (count <= (ClientList.size () - 1))
	{
		...
		if (otherclient != thisclient && otherclient->group == groupid)
		{
			...
			SendPM (thisclient, "%s - (GM %i, lvl %u), map %u[%.2f,%.2f], status %i", otherclient->CharInfo->charname, otherclient->Session->accesslevel, otherclient->Stats->Level,
				otherclient->Position->Map,
				otherclient->Position->current.x,
				otherclient->Position->current.y,
				statcheckbool);
		}
		count++;
	}
...
//Set the facing of an NPC because sometimes they are completely wrong
bool CWorldServer::pakSetNPCDir (CPlayer* thisclient, CNPC* thisNPC, float dir) {
	UINT map = thisclient->Position->Map;
	thisNPC->dir = dir;														//Sets the NPC facing direction
	thisclient->ClearObject (thisNPC->clientid);							//Unspawn the NPC at my client
	for (UINT i = 0; i < thisclient->VisibleNPCs.size (); i++) {				//Remove the NPCs clientid from my visibility list
		if (thisNPC == thisclient->VisibleNPCs.at (i)) {			//should re-appear at the next refresh cycle
			thisclient->VisibleNPCs.erase (thisclient->VisibleNPCs.begin () + i);
			break;
		}
	}
	//bool refresh = pakGMTele (thisclient, thisclient->Position->Map, thisclient->Position->current.x, thisclient->Position->current.y);
	return true;
}
void CWorldServer::pakMoveNPC (CPlayer* thisclient, CNPC* thisNPC) {
	UINT map = thisclient->Position->Map;

	thisNPC->pos = thisclient->Position->current;							//move the NPC to my current location
	thisclient->ClearObject (thisNPC->clientid);							//Unspawn the NPC at my client
	for (UINT i = 0; i < thisclient->VisibleNPCs.size (); i++) {				//Remove the NPCs clientid from my visibility list
		if (thisNPC == thisclient->VisibleNPCs.at (i)) {			//should re-appear at the next refresh cycle
			thisclient->VisibleNPCs.erase (thisclient->VisibleNPCs.begin () + i);
...
//save the direction of an NPC to the database once you are happy with it
void CWorldServer::pakSaveNPCDir (CNPC* thisNPC) {
	GServer->DB->QExecute ("UPDATE list_npcs SET dir=%f WHERE id='%i'", thisNPC->dir, thisNPC->id);
	return;
}

//save the direction and position of an NPC to the database once you are happy with it
void CWorldServer::pakSaveNPCPos (CNPC* thisNPC) {
	GServer->DB->QExecute ("UPDATE list_npcs SET x=%f, y=%f, dir=%f WHERE id='%i'", thisNPC->pos.x, thisNPC->pos.y, thisNPC->dir, thisNPC->id);
	return;
}

//Add a new NPC to the database once you are happy with it
void CWorldServer::pakAddNPC (CPlayer* thisclient, CNPC* thisNPC) {
	//GServer->DB->QExecute ("INSERT list_npcs SET x=%f, y=%f, dir=%f WHERE id='%i'", thisNPC->pos.x, thisNPC->pos.y, thisNPC->dir, thisNPC->id);
	GServer->DB->QExecute ("INSERT INTO list_npcs ( type, map, dir, x, y) VALUES (%i,'%i','%f',%f,'%f')",
		thisNPC->npctype, thisclient->Position->Map, thisNPC->dir, thisNPC->pos.x, thisNPC->pos.y);
...
bool CWorldServer::pakGMItemtoplayer(CPlayer* thisclient, char* name , UINT itemid, UINT itemtype, UINT itemamount, UINT itemrefine, UINT itemls, UINT itemstats, UINT itemsocket)
{
    ...
    item->itemnum        = itemid;
    item->itemtype        = itemtype;
    item->lifespan        = 99; //itemls Set lifespan to 99
    item->refine            = itemrefine;
    item->stats            = itemstats;
    item->socketed        = itemsocket;
    item->appraised        = 1;
...
bool CWorldServer::pakGMKillInRange( CPlayer* thisclient, int range )
{
    for(UINT j = 0; j < MapList.Index[thisclient->Position->Map]->MonsterList.size(); j++)
    {
        ...
        if( IsMonInCircle( thisclient, thismon, (float)range ))
        {
            ...
            SendToVisible( &pak, thismon );
        	CMap* map = MapList.Index[thisclient->Position->Map];
            map->DeleteMonster( thismon, true, j );
        }
    }
    return true;
}
...
bool CWorldServer::pakGMClass( CPlayer* thisclient, char* classid )
{
    ...
    */
    ...
    if ( changed && (donationbool || GM) )
    {
		...
		if (!GM)
		{
			SendPM(thisclient, "You paid 10VP for class change! to %s", classid );
		}
		else
		{
			SendPM (thisclient, "Class changed to %s", classid);
		}
        if(sp_points > 0)
        {
...
bool CWorldServer::pakFairyBuff (CPlayer* thisClient) 
{
	//so we called the buffme command. Now let's see if we are anywhere near a buff bot
	//first what map are we in?
	CMap* thisMap = GServer->MapList.Index[thisClient->Position->Map];
	//so now we know the map. let's search for buffbots in this map
	//initialize some stuff
	UINT buffpower = 0;
	for (DWORD j = 0; j < thisMap->NPCList.size (); j++)
	{
		CNPC* npc = thisMap->NPCList.at (j);
		if (npc->buffbot == true && npc->buffpower > 99)	//found a buffbot in this map. Strangely kept finding others besides teh buff fairy so added the second condition
		{
			Log (MSG_DEBUG, "found a buffbot in this map NPC id %i", npc->npctype);
			float dx = thisClient->Position->current.x - (float)(npc->pos.x);
			float dy = thisClient->Position->current.y - (float)(npc->pos.y);
	...
	{		//buffpower is still zero so we didn't find a buffbot in range
		SendPM (thisClient, "Sorry! there are no buff bots in range.");
	}
	else
	{		//Got buff bot so do the buffs
		pakGMGiveBuff (thisClient, thisClient, 3202, buffpower); 
		pakGMGiveBuff (thisClient, thisClient, 3203, buffpower);
		pakGMGiveBuff (thisClient, thisClient, 3204, buffpower);
		pakGMGiveBuff (thisClient, thisClient, 3205, buffpower);
		SendPM (thisClient, "Congratulations! You received buffs from the buff fairy with buffpower %i", buffpower);
	}
	return true;
}
...
//LMA: Mysql now.
//PY: Complete rewrite using data from the STBs. no more SQL
bool CWorldServer::pakGMAllSkill(CPlayer* thisclient, char* name)
{
	...
	//also clear the quickbar
	for (int i = 0; i < MAX_QUICKBAR; i++)
		otherclient->quickbar[i] = 0;

	/// PY: So.... Problem #1 is that the classid comes from a different table that the data used in list_skills.stb
	/// If we are going to load the skills directly from the STB then we are going to need a big ass switch statement
	bool result = false;
	switch (classid)
	{
		case 0:		//visitor so I don't get much
			result = GiveBasicSkills (otherclient);
			break;
		case 111:
			result = GiveSoldierSkills (otherclient);
			break;
		case 121:
			result = GiveKnightSkills (otherclient);
			break;
		case 122:
			result = GiveChampionSkills (otherclient);
			break;
		case 211:
			result = GiveMuseSkills (otherclient);
			break;
		case 221:
			result = GiveMageSkills (otherclient);
			break;
		case 222:
			result = GiveClericSkills (otherclient);
			break;
		case 311:
			result = GiveHawkerSkills (otherclient);
			break;
		case 321:
			result = GiveRaiderSkills (otherclient);
			break;
		case 322:
			result = GiveScoutSkills (otherclient);
			break;
		case 411:
			result = GiveDealerSkills (otherclient);
			break;
		case 421:
			result = GiveBorgeousSkills (otherclient);
			break;
		case 422:
			result = GiveArtisanSkills (otherclient);
			break;
	}
...
bool CWorldServer::GiveBasicSkills (CPlayer* thisclient)
{
	// All basic skills use skill tree tab 11
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->skill_tab == 11 && SkillList[i]->level == SkillList[i]->MaxLevel)	//level only equals max level when this instance of the skill is the highest one. No point giving the player a lesser version
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{ 
				SendPM (thisclient, "Command failed on skillid %i",SkillList[i]->id);
				return true;
			}
		}
	}
...
bool CWorldServer::GiveSoldierSkills (CPlayer* thisclient)
{
	//Soldier skills require job 41
	GiveBasicSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 41 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveKnightSkills (CPlayer* thisclient)
{
	//knight skills require job 61
	GiveSoldierSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 61 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveChampionSkills (CPlayer* thisclient)
{
	//Champion skills require job 62
	GiveSoldierSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 62 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveMuseSkills (CPlayer* thisclient)
{
	//muse skills require job 41
	GiveBasicSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 42 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveMageSkills (CPlayer* thisclient)
{
	//mage skills require job 63
	GiveMuseSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 63 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveClericSkills (CPlayer* thisclient)
{
	//cleric skills require job 64
	GiveMuseSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 64 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveDealerSkills (CPlayer* thisclient)
{
	//dealer skills require job 44
	GiveBasicSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 44 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveArtisanSkills (CPlayer* thisclient)
{
	//artisan skills require job 68
	GiveDealerSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 68 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveBorgeousSkills (CPlayer* thisclient)
{
	//bourgeous skills require job 67
	GiveDealerSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 67 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveHawkerSkills (CPlayer* thisclient)
{
	//hawker skills require job 43
	GiveBasicSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 43 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveScoutSkills (CPlayer* thisclient)
{
	//scout skills require job 66
	GiveHawkerSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 66 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveRaiderSkills (CPlayer* thisclient)
{
	//raider skills require job 65
	GiveHawkerSkills (thisclient);
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->c_class == 65 && SkillList[i]->level == SkillList[i]->MaxLevel)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveGMSkills (CPlayer* thisclient)
{
	//GM mastery skill requires skilltree tab of 2
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->skill_tab == 2)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
	...
	//now give the actual skills
	//requires tab 41 and required skill 3220
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->skill_tab == 41 && SkillList[i]->rskill[0] == 3220)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::GiveDevSkills (CPlayer* thisclient)
{
	return true;
}
bool CWorldServer::GiveDrivingSkills (CPlayer* thisclient)
{
	//driving skills require a skilltree tab of 30 - 39
	for (int i = 0; i < GServer->MaximumSkillID; i++)
	{
		if (SkillList[i]->skill_tab >= 30 && SkillList[i]->skill_tab <= 39)
		{
			if (!GiveSkill (thisclient, SkillList[i]->id, false))
			{
				SendPM (thisclient, "Command failed on skillid %i", SkillList[i]->id);
...
bool CWorldServer::pakGMSpawnForceRefresh(CPlayer* thisclient, UINT id, UINT map)
{
    ...
    for(UINT j=0;j<monstertokill.size();j++)
    {
        ...
        ADDDWORD   ( pak, 16 );
        SendToVisible( &pak, thismon );
        mymap->DeleteMonster( thismon, true, j );
    }

    ...
    switch( thisskill->buff[i])
    {
        case 16: // HP
        {
			if(thisskill->duration <= 0 )//is heal
            {
				...
				long int healpoint = (long int)thisskill->value1[i];		//Without this there is no default value and stuff like bonfire heals just don't get applied
                ...
                ///// PY. What is all this damn BS crap messing up my nice neat little function?????
				if (thisskill->id > 920 && thisskill->id < 926)//Cure (id.921-925)
                {
                    healpoint = (long int)(thisskill->value1[i] + ((Evalue - 15) * 1.5863));
                ...
                {
                    healpoint = (long int)(thisskill->value1[i] + ((Evalue - 15) * 3.6493));
                    character->Stats->HP += thisskill->value1[i] + healpoint;
                ...
                {
                    healpoint = (long int)(thisskill->value1[i] + ((Evalue - 15) * 1.7608));
                    character->Stats->HP += thisskill->value1[i] + healpoint;
                ...
                {
                    healpoint = (long int)(thisskill->value1[i] + ((Evalue - 15) * 3.3663));
                    character->Stats->HP += thisskill->value1[i] + healpoint;
                ...
                {
                    healpoint = (long int)(thisskill->value1[i] + ((Evalue - 15) * 9.2050));
                    character->Stats->HP += thisskill->value1[i] + healpoint;
                ...
                {
                    healpoint = (long int)(thisskill->value1[i] + ((Evalue - 15) * 4.1259));
                    character->Stats->HP += thisskill->value1[i] + healpoint;
                ...
                Log(MSG_INFO,"Character %i HP is now %I64i healed by %i HP", character->clientid, character->Stats->HP, healpoint);
                return true;
            }
        }
        break;
        case 17: // MP
        {
            if(thisskill->duration <= 0 )// recover MP
            {
                ...
                Log(MSG_INFO,"Character %i MP is now %I64i. restored by %i MP", character->clientid, character->Stats->MP, thisskill->value1[i]);
                return true;
            }
        }
        break;
...
bool CWorldServer::CheckABuffs( CSkills* thisskill, CCharacter* character, int Evalue ,int i)
{
    ...
    UINT randv = GServer->RandNumber(0, 100);

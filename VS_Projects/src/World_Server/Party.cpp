    ...
    switch(action)
    {
        ...
        case 0x01://invite a existent party
        {
            ...
            if ( otherclient->is_bot )
            {
                ...
                if( party == NULL )
                {
                    ...
                    AddParty( party );
                    thisclient->Party->IsMaster = true;
                }

                // Send accepted to inviter
                BEGINPACKET( pak, 0x7d1 );
                ADDBYTE    ( pak, 0x02 );
                ...
                // Send party level and exp
                RESETPACKET( pak, 0x7d4 );
                ADDBYTE    ( pak, party->PartyLevel );
                ADDDWORD   ( pak, party->Exp );
                thisclient->client->SendPacket( &pak );

                otherclient->Party->IsMaster = false;

                // Send new member info to party members
                RESETPACKET( pak, 0x7d2 );
                ADDBYTE    ( pak, party->Option );
                ADDBYTE    ( pak, 0x01 );
                ADDDWORD   ( pak, otherclient->CharInfo->charid );
                ADDWORD    ( pak, otherclient->clientid );
                ADDWORD    ( pak, otherclient->Stats->MaxHP );
                ADDWORD    ( pak, otherclient->Stats->HP );
                ADDDWORD   ( pak, BuildBuffs( otherclient ) );
                ADDDWORD   ( pak, 0x1f40008c );
                ADDWORD    ( pak, 0x1388 );
                ADDSTRING  ( pak, otherclient->CharInfo->charname );
                ADDBYTE    ( pak, 0x00 );
        ...
        case 0x02://Leave Party
        {
            ...
            if(!party->RemovePlayer( thisclient )) //if this player is not in this party
                return true;
            if( thisclient->is_bot && thisclient->bot_ai )
            {
                thisclient->bot_ai->SetFollowTarget( NULL );
                thisclient->bot_ai->SetState( BOT_STATE_IDLE );
            }
...
bool CWorldServer::pakPartyManager( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        case 0x02:										// Accept invitation
        {
            if(thisclient->Party->party != NULL)		// have party already
                return true;
            unsigned int clientid = GETWORD((*P),1);
            ...
            {
                BEGINPACKET( pak, 0x7d1 );
                ADDBYTE    ( pak, 0x00 );				// Cannot find the player who invited
                ADDWORD    ( pak, clientid );
                ADDBYTE    ( pak, 0x00 );
                thisclient->client->SendPacket( &pak );
                return true;
            }
            CParty* party = otherclient->Party->party;	// Cast local party object as the inviter's party
            if(party != NULL)
            {
                party->RefreshMax();					//LMA: Refreshing Capacity if needed
            ...
            {
                BEGINPACKET( pak, 0x7d1 );
                ADDBYTE    ( pak, 0x07 );				// Level inapropriate
                ADDWORD    ( pak, clientid );
                ADDBYTE    ( pak, 0x00 );
                thisclient->client->SendPacket( &pak );
                return true;
            ...
            BEGINPACKET( pak, 0x7d1 );
            ADDBYTE    ( pak, 0x02 );					// Accepted to Party
            ADDWORD    ( pak, otherclient->clientid );
            ADDBYTE    ( pak, 0x00 );
            otherclient->client->SendPacket( &pak );
            if( party == NULL )
...
unsigned int CWorldServer::GetMaxPartyExp( unsigned int partylevel )
{
	...
	//return ((partylevel + 7) * (partylevel + 10) * 5) + 200;			///137 method
	unsigned int exp = ((288 * (partylevel - 1)) + 256 + ((((partylevel - 2) * (partylevel - 1)) / 2) * 32));	///CrazyCabling method. included within 137 tags because if these tags are set false then auto decrease fails
	Log (MSG_INFO, "Party exp %u for party level %u", exp, partylevel);
	return exp;
#else
	unsigned int exp = 540;							///old method
...
#ifdef USE_137_PARTY_EXP
void CWorldServer::UpdatePartyExp (CParty* party)
{
	clock_t etime = clock () - party->next_exp_update;
	if (etime > AutoExpDecrease)							// value set in Worldserver.h at 10,000
	{
		DWORD TempDecrease = GetAutoDecreaseExp (party->PartyLevel, party->Members.size ()) * etime / AutoExpDecrease;
		if (TempDecrease > party->Exp)		//this is to stop the decrease calculation from going negative. It's unsigned so that would result in a really bad day
		{
			party->Exp = 0;					// We have hit rock bottom. Luckily we can't actually lose a level
		}
		else
		{
			party->Exp -= TempDecrease;
			//SetExp (m_iCurrExp - (GetAutoDecreaseExp (m_iLevel, GetMemberCount ()) * dwTimeSpace) / c_iAutoExpDecreaseTimeSpace); //PY Formula from client 
		}
		party->next_exp_update = clock ();
	}
}

int CWorldServer::GetAutoDecreaseExp (int Partylevel, int MemberCount)
{
	if (MemberCount <= 0)
		return 0;

	return Partylevel / 2 + MemberCount;
}
#endif

...
AIACT(012)
{
    ...
    //PY This is bullshit. AIP exist to control the behaviour of the monster. If AIP says it attacks then it damn well attacks
...
AIACT(017)
{
	...
	newdrop->item = newdrop->item->GetCItem (newdrop->item->itemtype, newdrop->item->itemnum);	// get data for the base version of the item
...
AIACT(034)
{
	...
	UINT itemtype = GServer->gi(nItem,0);
	UINT itemnum = GServer->gi(nItem,1);
	CItem* thisitem = new (nothrow) CItem;
	thisitem = thisitem->GetCItem (itemtype, itemnum);
	//thisitem.Clear();		//PY initializes the item to set default perameters

...
CNPC* CMap::GetNPCInMapByType (UINT npcType) 
{
	for (UINT i = 0; i < NPCList.size(); i++)
	{
		if (NPCList.at (i)->npctype == npcType)
			return NPCList.at (i);
	}
	return NULL;
}
...
void CMap::RespawnMonster( )
{
    ...
    //Way more elegant that all that convoluted LMA stuff ^_^
    //PY END
}

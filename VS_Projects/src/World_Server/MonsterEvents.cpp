...
bool CMonster::OnDie( )
{
    ...
    //UINT special_exp=0;
    //UINT special_lvl=0;
	GServer->GiveExp (this);		/// PY: no longer needs these values.     , special_lvl, special_exp );
...
//bool CMonster::OnEnemyOnSight( CPlayer* Enemy )					///PY This function is NEVER called. Mainly because it's BULLSHIT. AIP all the way baby!!!
//{
...
bool CMonster::SummonUpdate(CMonster* monster, CMap* map, UINT j)
{
    ...
    if( etime >= 4 * CLOCKS_PER_SEC && Stats->HP > 0 )
    {
        ...
        {
            //LMA: We update the gauge, only if it was a skill summon
            if(ownerclient != NULL&&monster->owner_user_id>0)
            {
                ownerclient->cur_gauge-=GServer->NPCData[monster->montype]->tab1;
                if(ownerclient->cur_gauge<=0)
                {
                    ownerclient->cur_gauge=0;
                }

                Log(MSG_INFO,"summon died (took %u), gauge is now %i/%i",GServer->NPCData[monster->montype]->tab1,ownerclient->cur_gauge,ownerclient->summon_gauge);
            }
            else
            {
                ...
                if(monster->owner_user_id>0)
                {
                    ...
                    if(tempplayer!=NULL)
                    {
                        tempplayer->cur_gauge-=GServer->NPCData[monster->montype]->tab1;
                        if(tempplayer->cur_gauge<=0)
                        {
                            tempplayer->cur_gauge=0;
                        }

                        Log(MSG_INFO,"summon died (owner found by userid) (took %u), gauge is now %i/%i",GServer->NPCData[monster->montype]->tab1,tempplayer->cur_gauge,tempplayer->summon_gauge);
                    }
            ...
            //map->DeleteMonster( monster, true, j );			//PY let's not do this here
            return true;
        }
        else
        {
    ...
    {
        CPlayer* ownerclient = monster->GetOwner( );
        //LMA: We update the gauge (only for skill summons)
        if(ownerclient != NULL&&monster->owner_user_id>0)
        {
            ownerclient->cur_gauge-=GServer->NPCData[monster->montype]->tab1;
            if(ownerclient->cur_gauge<=0)
        ...
        else
        {
            ...
            if(monster->owner_user_id>0)
            {
                ...
                if(tempplayer!=NULL)
                {
                    ...
                    Log(MSG_INFO,"summon died (owner found by userid 2) (took %u), gauge is now %i/%i",GServer->NPCData[monster->montype]->tab1,tempplayer->cur_gauge,tempplayer->summon_gauge);
                }
